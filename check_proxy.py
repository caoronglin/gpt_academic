...
def check_proxy(proxies, return_ip=False):
    ...
    try:
        response = requests.get(
            "https://ipapi.co/json/", proxies=proxies, timeout=4
        )  # ⭐ 执行GET请求以获取代理信息
        data = response.json()
        if "country_name" in data:
            ...
            alternative, ip = _check_with_backup_source(
                proxies
            )  # ⭐ 调用备用方法检查代理配置
...
def _check_with_backup_source(proxies):
    ...
    try:
        res_json = requests.get(
            f"http://{random_string}.edns.ip-api.com/json", proxies=proxies, timeout=4
        ).json()  # ⭐ 执行代理检查和备份源请求
...
def backup_and_download(current_version, remote_version):
    ...
    try:
        ...
        r = requests.get(
            "https://public.agent-matrix.com/publish/master.zip",
            proxies=proxies,
            stream=True,
        )
...
def patch_and_restart(path):
    ...
    from shared_utils.colorful import log亮红, log亮绿, log亮黄

    if not os.path.exists("config_private.py"):
        log亮黄(
            "由于您没有设置config_private.py私密配置，现将您的现有配置移动至config_private.py以防止配置丢失，",
...
def get_current_version():
    ...
    try:
        with open("./version", "r", encoding="utf8") as f:
            current_version = json.loads(f.read())[
                "version"
            ]  # ⭐ 从读取的json数据中提取版本号
    except:
        current_version = ""
...
def auto_update(raise_error=False):
    ...
    try:
        ...
        try:
            response = requests.get(
                "https://raw.githubusercontent.com/binary-husky/chatgpt_academic/master/version",
                proxies=proxies,
                timeout=5,
            )
        except:
            response = requests.get(
                "https://public.agent-matrix.com/publish/version",
                proxies=proxies,
                timeout=5,
            )
        ...
        if (remote_version - current_version) >= 0.01 - 1e-5:
            ...
            user_instruction = input(
                "（2）是否一键更新代码（Y+回车=确认，输入其他/无输入+回车=不更新）？"
            )
            if user_instruction in ["Y", "y"]:
                path = backup_and_download(
