...
def load_web_cookie_cache__fn_builder(
    ...
    def load_web_cookie_cache(persistent_cookie_, cookies_):
        ...
        for k, v in persistent_cookie_["custom_bnt"].items():
            ...
            if k in customize_btns:
                ret.update(
                    {customize_btns[k]: gr.update(visible=True, value=v["Title"])}
                )
            else:
                ret.update(
                    {predefined_btns[k]: gr.update(visible=True, value=v["Title"])}
                )
        return ret
            ...
            ret.update(
                {
                    customize_btns[basic_btn_dropdown_]: gr.update(
                        visible=visible, value=basic_fn_title
                    )
                }
            )
        else:
...
def create_button_with_javascript_callback(
    btn_value,
    elem_id,
    variant,
    js_callback,
    ...
    def get_fn_wrap():
        def fn_wrap(*args):
            ...
            summary = base64.b64encode(json.dumps(summary_dict).encode("utf8")).decode(
                "utf-8"
            )
            return (*res, summary)
