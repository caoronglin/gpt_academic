    ...
    try:
        if isinstance(default_value, bool):
            ...
            if env_arg == "True":
                ...
                log亮红("Expect `True` or `False`, but have:", env_arg)
                r = default_value
...
def read_single_conf_with_lru_cache(arg):
    ...
    try:
        # 优先级1. 获取环境变量作为配置
        default_ref = getattr(
            importlib.import_module("config"), arg
        )  # 读取默认值作为数据类型转换的参考
    ...
    if arg == "API_URL_REDIRECT":
        ...
        )  # API_URL_REDIRECT填写格式是错误的，请阅读`https://github.com/binary-husky/gpt_academic/wiki/项目配置说明`
    ...
    if arg == "API_KEY":
        ...
        if is_any_api_key(r):
            ...
            log亮红(
                f"[API_KEY] 您的 API_KEY（{r[:15]}***）不满足任何一种已知的密钥格式，请在config文件中修改API密钥之后再运行（详见`https://github.com/binary-husky/gpt_academic/wiki/api_key`）。"
            )
    if arg == "proxies":
        if not read_single_conf_with_lru_cache("USE_PROXY"):
            r = None  # 检查USE_PROXY，防止proxies单独起作用
        if r is None:
            log亮红(
