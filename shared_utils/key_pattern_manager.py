...
def is_any_api_key(key):
    ...
    if "," in key:
        ...
        return (
            is_openai_api_key(key)
            or is_api2d_key(key)
            or is_azure_api_key(key)
            or is_cohere_api_key(key)
        )
...
def select_api_key(keys, llm_model):
    ...
    if (
        llm_model.startswith("gpt-")
        or llm_model.startswith("chatgpt-")
        or llm_model.startswith("one-api-")
        or is_o_family_for_openai(llm_model)
    ):
        for k in key_list:
            if is_openai_api_key(k):
