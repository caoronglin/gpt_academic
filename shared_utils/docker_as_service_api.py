...
class DockerServiceApiComModel(BaseModel):
    client_command: Optional[str] = Field(
        default=None,
        title="Client command",
        description="The command to be executed on the client side",
    )
    client_file_attach: Optional[dict] = Field(
        default=None,
        title="Client file attach",
        description="The file to be attached to the client side",
    )
    server_message: Optional[Any] = Field(
        default=None,
        title="Server standard error",
        description="The standard error from the server side",
    )
    server_std_err: Optional[str] = Field(
        default=None,
        title="Server standard error",
        description="The standard error from the server side",
    )
    server_std_out: Optional[str] = Field(
        default=None,
        title="Server standard output",
        description="The standard output from the server side",
    )
    server_file_attach: Optional[dict] = Field(
        default=None,
        title="Server file attach",
        description="The file to be attached to the server side",
    )


def process_received(
    received: DockerServiceApiComModel,
    save_file_dir="./daas_output",
...
def stream_daas(docker_service_api_com_model, server_url, save_file_dir):
    ...
    files = {
        "file": (
            "docker_service_api_com_model.pkl",
            file_obj,
        ...
        for chunk in response.iter_content(max_full_package_size):
            if chunk:
                ...
                try:
                    ...
                    chunk_buf = None
                    received_output_manifest = process_received(
                        received,
                        save_file_dir,
                        output_manifest=received_output_manifest,
                    )
