...
def auto_context_clip_each_message(current, history):
    ...
    # compute rank
    clip_prior_weight = [
        (token_num / clip_token_len + (len(context) - index) * 0.1)
        for index, token_num in enumerate(context_token_num)
    ]
    # print('clip_prior_weight', clip_prior_weight)
    # get sorted index of context_token_num, from largest to smallest
    sorted_index = sorted(
        range(len(context_token_num)), key=lambda k: clip_prior_weight[k], reverse=True
    )

    # pre compute space yield
    for index in sorted_index:
        print(
            "index", index, f"current total {total_token_num}, target {clip_token_len}"
        )
        ...
        if context_token_num[index] < allowed_token_num_this_entry:
            print(
                ...
                index,
                "[allowed] before",
                context_token_num[index],
                "allowed",
                allowed_token_num_this_entry,
            )
            continue
...
def auto_context_clip_search_optimal(
    current, history, promote_latest_long_message=False
):
    """
    current: 当前消息
    history: 历史消息列表
