...
def lru_file_cache(maxsize=128, ttl=None, filename=None):
    ...
    """
    cache_path = (
        os.path.join(CACHE_FOLDER, f"{filename}.cache")
    ...
    def decorator_function(func):
        ...
        def wrapper_function(*args, **kwargs):
            ...
            if key in cache:
                if (
                    _cache_info["ttl"] is None
                    or (cache[key][1] + _cache_info["ttl"]) >= time.time()
                ):
                    _cache_info["hits"] += 1
                    print(
                        f"Warning, reading cache, last read {(time.time()-cache[key][1])//60} minutes ago"
                    )
                    time.sleep(2)
...
def read_map_from_json(language):
    if os.path.exists(f"docs/translate_{language.lower()}.json"):
        with open(f"docs/translate_{language.lower()}.json", "r", encoding="utf8") as f:
            res = json.load(f)
            res = {
                k: v for k, v in res.items() if v is not None and contains_chinese(k)
            }
            return res
    return {}
...
def trans(word_to_translate, language, special=False):
    ...
    if special:  #  to English using CamelCase Naming Convention
        sys_prompt_array = [
            f"Translate following names to English with CamelCase naming convention. Keep original format"
            for _ in inputs_array
        ]
    else:
        sys_prompt_array = [
            f"Translate following sentences to {LANG}. E.g., You should translate sentences to the following format ['translation of sentence 1', 'translation of sentence 2']. Do NOT answer with Chinese!"
            for _ in inputs_array
        ]
    ...
    for i, r in enumerate(result):
        if i % 2 == 1:
            try:
                ...
                # except:
                print(
                    "GPT answers with unexpected format, some words may not be translated, but you can try again later to increase translation coverage."
                )
                res_before_trans = eval(result[i - 1])
...
def step_1_core_key_translate():
            ...
            for node in ast.walk(root):
                if isinstance(node, ast.Name):
                    if contains_chinese(node.id):
                        syntax.append(node.id)
                if isinstance(node, ast.Import):
                    for n in node.names:
                        if contains_chinese(n.name):
                            syntax.append(n.name)
    ...
    for d in chinese_core_keys:
        if d not in chinese_core_keys_norepeat:
            chinese_core_keys_norepeat.append(d)
    need_translate = []
    ...
    cached_translation = dict(
        sorted(cached_translation.items(), key=lambda x: -len(x[0]))
    )
    ...
    chinese_core_keys_norepeat_mapping = dict(
        sorted(chinese_core_keys_norepeat_mapping.items(), key=lambda x: -len(x[0]))
    )
    ...
    def copy_source_code():
        ...
        try:
            shutil.rmtree(f"./multi-language/{LANG}/")
        except:
            pass
...
def step_2_core_key_translate():
    ...
    def load_string(strings, string_input):
        ...
        # --------------------------------------
        splitted_string = advanced_split(
            splitted_string, spliter="，", include_spliter=False
        )
        splitted_string = advanced_split(
            splitted_string, spliter="。", include_spliter=False
        )
    ...
    for d in chinese_literal_names:
        if d not in chinese_literal_names_norepeat:
            chinese_literal_names_norepeat.append(d)
    need_translate = []
    cached_translation = read_map_from_json(language=LANG)
    cached_translation_keys = list(cached_translation.keys())
