...
deepseek_nwafu_endpoint = "https://deepseek.nwafu.edu.cn/api/chat/completions"
...
if deepseek_nwafu_endpoint in API_URL_REDIRECT:
    deepseek_nwafu_endpoint = API_URL_REDIRECT[deepseek_nwafu_endpoint]
if grok_model_endpoint in API_URL_REDIRECT:
    grok_model_endpoint = API_URL_REDIRECT[grok_model_endpoint]
if volcengine_endpoint in API_URL_REDIRECT:
    volcengine_endpoint = API_URL_REDIRECT[volcengine_endpoint]
...
# -=-=-=-=-=-=- 西北农林科技大学DeepSeek平台 -=-=-=-=-=-=-
deepseek_nwafu_models = [
    "deepseek-nwafu-chat",
    "deepseek-nwafu-coder",
    "deepseek-nwafu-reasoner",
]
if any(item in deepseek_nwafu_models for item in AVAIL_LLM_MODELS):
    try:
        model_info.update(
            {
                "deepseek-nwafu-chat": {
                    "fn_with_ui": deepseek_nwafu_ui,
                    "fn_without_ui": deepseek_nwafu_noui,
                    "endpoint": deepseek_nwafu_endpoint,
                    "can_multi_thread": True,
                    "max_token": 64000,
                    "tokenizer": tokenizer_gpt35,
                    "token_cnt": get_token_num_gpt35,
...
for model in [m for m in AVAIL_LLM_MODELS if m.startswith("volcengine-")]:
    ...
    try:
        ...
        # 如果是已知模型，则尝试获取其信息
        original_model_info = model_info_extend.get(
            origin_model_name.replace("volcengine-", "", 1), None
        )
    except:
        logger.error(
            f"volcengine模型 {model} 的 max_token 配置不是整数，请检查配置文件。"
        )
        continue

    volcengine_noui, volcengine_ui = get_predict_function(
        api_key_conf_name="ARK_API_KEY",
        max_output_token=8192,
        disable_proxy=True,
        model_remove_prefix=["volcengine-"],
    )
...
def LLM_CATCH_EXCEPTION(f):
    ...
    def decorated(
        ...
        try:
            return f(
                inputs, llm_kwargs, history, sys_prompt, observe_window, console_silence
            )
