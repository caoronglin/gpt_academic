...
if "Connect_OpenAI" not in WHEN_TO_USE_PROXY:
    if proxies is not None:
        logger.error(
            "虽然您配置了代理设置，但不会在连接OpenAI的过程中起作用，请检查WHEN_TO_USE_PROXY配置。"
        )
        proxies = None
...
def make_multimodal_input(inputs, image_paths):
    ...
    for image_path in image_paths:
        ...
        inputs = (
            inputs
            + f'<br/><br/><div align="center"><img src="file={path}" base64="{base64}"></div>'
        )
...
def append_image_if_contain_base64(inputs):
    if not contain_base64(inputs):
        ...
        res = []
        res.append({"type": "text", "text": inputs})
...
def predict_no_ui_long_connection(
    ...
    headers, payload = generate_payload(
        inputs, llm_kwargs, history, system_prompt=sys_prompt, stream=stream
    )
    retry = 0
    while True:
        try:
    ...
    while True:
        ...
        chunk_decoded, chunkjson, has_choices, choice_valid, has_content, has_role = (
            decode_chunk(chunk)
        )
        ...
        if len(delta) == 0:
            ...
            if (has_choices) and (
                chunkjson["choices"][0].get("finish_reason", "null") == "stop"
            ):
                is_termination_certain = True
        ...
        if (not has_content) and (not has_role):
            continue  # raise RuntimeError("发现不标准的第三方接口："+delta)
...
def predict(
    ...
    if is_any_api_key(inputs):
        ...
        yield from update_ui(
            chatbot=chatbot, history=history, msg="api_key已导入"
        )  # 刷新界面
        return
        ...
        yield from update_ui(
            chatbot=chatbot, history=history, msg="缺少api_key"
        )  # 刷新界面
        return
    ...
    # 多模态模型
    has_multimodal_capacity = model_info[llm_kwargs["llm_model"]].get(
    ...
    try:
        ...
        yield from update_ui(
            chatbot=chatbot, history=history, msg="api-key不满足要求"
        )  # 刷新界面
        return
    ...
    try:
        ...
        yield from update_ui(
            chatbot=chatbot, history=history, msg="Endpoint不满足要求"
        )  # 刷新界面
        return
    ...
    if not stream:
        # 该分支仅适用于不支持stream的o1模型，其他情形一律不适用
        yield from handle_o1_model_special(
            response, inputs, llm_kwargs, chatbot, history
        )
        return
        ...
        while True:
            try:
                ...
                if len(gpt_replying_buffer.strip()) > 0 and len(error_msg) == 0:
                    ...
                    break
                # 其他情况，直接返回报错
                chatbot, history = handle_error(
                    inputs, llm_kwargs, chatbot, history, chunk_decoded, error_msg
                )
                yield from update_ui(
                    chatbot=chatbot,
                    history=history,
                    msg="接口返回了错误:" + chunk.decode(),
                )  # 刷新界面
                return
            ...
            if (
                ...
                and (r'"object":"error"' not in chunk_decoded)
                and (r"content" not in chunk_decoded)
            ):
                # 数据流的第一帧不携带content
            ...
            if "error" in chunk_decoded:
                logger.error(f"接口返回了未知错误: {chunk_decoded}")
            ...
            if chunk:
                try:
                    if has_choices and not choice_valid:
                        ...
                        continue
                    if (
                        ("data: [DONE]" not in chunk_decoded)
                        and len(chunk_decoded) > 0
                        and (chunkjson is None)
                    ):
                        # 传递进来一些奇怪的东西
                        raise ValueError(
                            f"无法读取以下数据，请检查配置。\n\n{chunk_decoded}"
                    ...
                    openai_terminate = (has_choices) and (
                        len(chunkjson["choices"][0]["delta"]) == 0
                    )
                    if one_api_terminate or openai_terminate:
                        is_termination_certain = False
                        if one_api_terminate:
                            is_termination_certain = True  # 抓取符合规范的结束条件
                        elif (has_choices) and (
                            chunkjson["choices"][0].get("finish_reason", "null")
                            == "stop"
                        ):
                            is_termination_certain = True  # 抓取符合规范的结束条件
                        if is_termination_certain:
                    ...
                    try:
                        ...
                        logger.error(
                            f"一些第三方接口出现这样的错误，兼容一下吧: {chunk_decoded}"
                        )
                    # 处理数据流的主体
                    if has_content:
                        # 正常情况
                        ...
                        if (
                            chunkjson["choices"][0]["delta"].get("content", None)
                            is None
                        ):
                            logger.error(
                                f"一些第三方接口出现这样的错误，兼容一下吧: {chunk_decoded}"
                    ...
                    yield from update_ui(
                        chatbot=chatbot, history=history, msg="Json解析异常" + error_msg
                    )  # 刷新界面
                    return
...
def handle_o1_model_special(response, inputs, llm_kwargs, chatbot, history):
    try:
        ...
        yield from update_ui(
            chatbot=chatbot, history=history, msg="Json解析异常" + response.text
        )  # 刷新界面
...
def handle_error(inputs, llm_kwargs, chatbot, history, chunk_decoded, error_msg):
    ...
    if "reduce the length" in error_msg:
        ...
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] Your account is not active. OpenAI以账户失效为由, 拒绝服务."
            + openai_website,
        )
    elif "associated with a deactivated account" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] You are associated with a deactivated account. OpenAI以账户失效为由, 拒绝服务."
            + openai_website,
        )
    elif "API key has been deactivated" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] API key has been deactivated. OpenAI以账户失效为由, 拒绝服务."
            + openai_website,
        )
    elif "bad forward key" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] Bad forward key. API2D账户额度不足.",
        )
...
def generate_payload(
    ...
    if llm_kwargs["llm_model"].startswith("azure-"):
        ...
        if llm_kwargs["llm_model"] in AZURE_CFG_ARRAY.keys():
            azure_api_key_unshared = AZURE_CFG_ARRAY[llm_kwargs["llm_model"]][
                "AZURE_API_KEY"
            ]
    ...
    openai_disable_system_prompt = model_info[llm_kwargs["llm_model"]].get(
        "openai_disable_system_prompt", False
    )
    ...
    if not enable_multimodal_capacity:
        ...
        if conversation_cnt:
            for index in range(0, 2 * conversation_cnt, 2):
                ...
                what_i_have_asked["content"] = remove_image_if_contain_base64(
                    history[index]
                )
                what_gpt_answer = {}
                what_gpt_answer["role"] = "assistant"
                what_gpt_answer["content"] = remove_image_if_contain_base64(
                    history[index + 1]
                )
        ...
        if conversation_cnt:
            for index in range(0, 2 * conversation_cnt, 2):
                what_i_have_asked = {}
                what_i_have_asked["role"] = "user"
                what_i_have_asked["content"] = append_image_if_contain_base64(
                    history[index]
                )
                what_gpt_answer = {}
                what_gpt_answer["role"] = "assistant"
                what_gpt_answer["content"] = append_image_if_contain_base64(
                    history[index + 1]
                )
                if what_i_have_asked["content"] != "":
                    if what_gpt_answer["content"] == "":
    ...
    openai_force_temperature_one = model_info[llm_kwargs["llm_model"]].get(
