...
from toolbox import ChatBotWithCookies, get_conf, log_chat, update_ui
...
class MoonShotInit:
    ...
    def __conversation_history(self, history):
        ...
        if conversation_cnt:
            for index in range(0, 2 * conversation_cnt, 2):
                what_i_have_asked = {"role": "user", "content": str(history[index])}
                what_gpt_answer = {
                    "role": "assistant",
    ...
    def generate_messages(self, inputs, llm_kwargs, history, system_prompt, stream):
        payload, headers = self.generate_payload(
...
def msg_handle_error(llm_kwargs, chunk_decoded):
    ...
    if "does not exist" in chunk_decoded:
        ...
        error_msg = (
            f"[Local Message] Incorrect API key. OpenAI以提供了不正确的API_KEY为由, 拒绝服务."
            + openai_website
        )
        ...
        try:
            error_msg = json.dumps(
                json.loads(chunk_decoded[:6]), indent=4, ensure_ascii=False
            )
        except:
...
def predict(
    ...
    stream_response = gpt_bro_init.generate_messages(
        inputs, llm_kwargs, history, system_prompt, stream
    )
    for content, gpt_bro_result, error_bro_meg in stream_response:
        ...
        if error_bro_meg:
            ...
            break
    log_chat(
        llm_model=llm_kwargs["llm_model"], input_str=inputs, output_str=gpt_bro_result
    )
...
def predict_no_ui_long_connection(
    ...
    stream_response = gpt_bro_init.generate_messages(
        inputs, llm_kwargs, history, sys_prompt, True
    )
    moonshot_bro_result = ""
...
if __name__ == "__main__":
    ...
    for g in moon_ai.generate_messages(
        "hello", {"llm_model": "moonshot-v1-8k"}, [], "", True
    ):
        print(g)
