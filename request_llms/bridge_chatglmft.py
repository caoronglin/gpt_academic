...
def string_to_options(arguments):
    ...
    parser.add_argument(
        "--llm_to_learn", type=str, help="LLM model to learn", default="gpt-3.5-turbo"
    )
...
class GetGLMFTHandle(Process):
    ...
    def run(self):
        ...
        while True:
            try:
                if self.chatglmft_model is None:
                    ...
                    logger.info(
                        f"Loading prefix_encoder weight from {CHATGLM_PTUNING_CHECKPOINT}"
                    )
                    ...
                    prefix_state_dict = torch.load(
                        os.path.join(CHATGLM_PTUNING_CHECKPOINT, "pytorch_model.bin")
                    )
                    new_prefix_state_dict = {}
                    for k, v in prefix_state_dict.items():
                        if k.startswith("transformer.prefix_encoder."):
                            new_prefix_state_dict[
                                k[len("transformer.prefix_encoder.") :]
                            ] = v
                    ...
                    if (
                        ...
                        and model_args["quantization_bit"] != 0
                    ):
                        logger.info(
                            f"Quantized to {model_args['quantization_bit']} bit"
                        )
                ...
                if retry > 3:
                    self.child.send(
                        "[Local Message] Call ChatGLMFT fail 不能正常加载ChatGLMFT的参数。"
                    )
        ...
        while True:
            ...
            # 收到消息，开始请求
            try:
                for response, history in self.chatglmft_model.stream_chat(
                    self.chatglmft_tokenizer, **kwargs
                ):
                ...
                self.child.send(
                    "[Local Message] Call ChatGLMFT fail."
                    + "\n```\n"
                    + trimmed_format_exc()
                    + "\n```\n"
                )
...
def predict_no_ui_long_connection(
    ...
    response = ""
    for response in glmft_handle.stream_chat(
        query=inputs,
