...
class GetGLMHandle(Process):
    ...
    def moss_init(self):  # 子进程执行
        ...
        if (
            args.model_name
            in ["fnlp/moss-moon-003-sft-int8", "fnlp/moss-moon-003-sft-int4"]
            and num_gpus > 1
        ):
            raise ValueError(
                "Quantized models do not support model parallel. Please run on a single GPU (e.g., --gpu 0) or use `fnlp/moss-moon-003-sft`"
            )
...
def predict_no_ui_long_connection(
    ...
    response = ""
    for response in moss_handle.stream_chat(
        query=inputs,
