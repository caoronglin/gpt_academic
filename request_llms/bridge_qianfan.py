...
def get_access_token():
    ...
    BAIDU_CLOUD_API_KEY, BAIDU_CLOUD_SECRET_KEY = get_conf(
        "BAIDU_CLOUD_API_KEY", "BAIDU_CLOUD_SECRET_KEY"
    )

    if len(BAIDU_CLOUD_SECRET_KEY) == 0:
        raise RuntimeError("没有配置BAIDU_CLOUD_SECRET_KEY")
    if len(BAIDU_CLOUD_API_KEY) == 0:
        raise RuntimeError("没有配置BAIDU_CLOUD_API_KEY")

    url = "https://aip.baidubce.com/oauth/2.0/token"
    params = {
        "grant_type": "client_credentials",
        "client_id": BAIDU_CLOUD_API_KEY,
        "client_secret": BAIDU_CLOUD_SECRET_KEY,
    }
...
def generate_message_payload(inputs, llm_kwargs, history, system_prompt):
    ...
    if conversation_cnt:
        for index in range(0, 2 * conversation_cnt, 2):
            ...
            what_i_have_asked["content"] = (
                history[index] if history[index] != "" else "Hello"
            )
            what_gpt_answer = {}
            what_gpt_answer["role"] = "assistant"
            what_gpt_answer["content"] = (
                history[index + 1] if history[index] != "" else "Hello"
            )
...
def generate_from_baidu_qianfan(inputs, llm_kwargs, history, system_prompt):
    ...
    payload = json.dumps(
        {
            "messages": generate_message_payload(
...
def predict_no_ui_long_connection(
    ...
    for response in generate_from_baidu_qianfan(
