...
def predict(
    ...
    if not have_recent_file:
        chatbot.append(
            (
                inputs,
                "没有检测到任何近期上传的图像文件，请上传jpg格式的图片，此外，请注意拓展名需要小写",
            )
        )
        yield from update_ui(
            chatbot=chatbot, history=history, msg="等待图片"
        )  # 刷新界面
        return
    if os.path.exists(inputs):
        chatbot.append(
            (
                inputs,
                "已经接收到您上传的文件，您不需要再重复强调该文件的路径了，请直接输入您的问题。",
            )
        )
        yield from update_ui(
            chatbot=chatbot, history=history, msg="等待指令"
        )  # 刷新界面
        return
    ...
    try:
        headers, payload, api_key = generate_payload(
            inputs, llm_kwargs, history, system_prompt, image_paths
        )
                ...
                try:
                    ...
                    if ("data: [DONE]" in chunk_decoded) or (
                        ...
                        yield from update_ui_latest_msg(
                            lastmsg, chatbot, history, delay=1
                        )
                        log_chat(
                    ...
                    yield from update_ui(
                        chatbot=chatbot, history=history, msg="Json异常" + error_msg
                    )  # 刷新界面
...
def handle_error(
    inputs, llm_kwargs, chatbot, history, chunk_decoded, error_msg, api_key=""
):
    from .bridge_all import model_info
