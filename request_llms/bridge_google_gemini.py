...
def predict(
    ...
    if get_conf("GEMINI_API_KEY") == "":
        yield from update_ui_latest_msg(
            f"请配置 GEMINI_API_KEY。", chatbot=chatbot, history=history, delay=0
        )
        return
    ...
    while True:
        try:
            stream_response = genai.generate_chat(
                inputs,
                llm_kwargs,
                history,
                system_prompt,
            ...
            yield from update_ui(
                chatbot=chatbot, history=history, msg="请求失败"
            )  # 刷新界面
            return
    gpt_replying_buffer = ""
    gpt_security_policy = ""
...
if __name__ == "__main__":
    ...
    result = predict(
        "Write long a story about a magic backpack.", llm_kwargs, llm_kwargs, []
    )
    for i in result:
        print(i)
