        ...
        if chunk:
            try:
                ...
                if not console_silence:
                    print(chunkjson["message"]["content"], end="")
        ...
        while True:
            ...
            if chunk:
                try:
                    ...
                    gpt_replying_buffer = (
                        gpt_replying_buffer + chunkjson["message"]["content"]
                    )
...
def handle_error(inputs, llm_kwargs, chatbot, history, chunk_decoded, error_msg):
    ...
    if "bad_request" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] 已经超过了模型的最大上下文或是模型格式错误,请尝试削减单次输入的文本量。",
        )
    elif "authentication_error" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] Incorrect API key. 请确保API key有效。",
        )
    elif "not_found" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            f"[Local Message] {llm_kwargs['llm_model']} 无效，请确保使用小写的模型名称。",
        )
    elif "rate_limit" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] 遇到了控制请求速率限制，请一分钟后重试。",
        )
