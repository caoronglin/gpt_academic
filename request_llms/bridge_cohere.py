        ...
        while True:
            try:
                ...
                yield from update_ui(
                    chatbot=chatbot,
                    history=history,
                    msg="非Cohere官方接口返回了错误:" + chunk.decode(),
                )  # 刷新界面
                return
...
def handle_error(inputs, llm_kwargs, chatbot, history, chunk_decoded, error_msg):
    ...
    if "reduce the length" in error_msg:
        ...
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] Incorrect API key. Cohere以提供了不正确的API_KEY为由, 拒绝服务. "
            + Cohere_website,
        )
    elif "exceeded your current quota" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] You exceeded your current quota. Cohere以账户额度不足为由, 拒绝服务."
            + Cohere_website,
        )
    elif "account is not active" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] Your account is not active. Cohere以账户失效为由, 拒绝服务."
            + Cohere_website,
        )
    elif "associated with a deactivated account" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] You are associated with a deactivated account. Cohere以账户失效为由, 拒绝服务."
            + Cohere_website,
        )
    elif "API key has been deactivated" in error_msg:
        chatbot[-1] = (
            chatbot[-1][0],
            "[Local Message] API key has been deactivated. Cohere以账户失效为由, 拒绝服务."
            + Cohere_website,
        )
...
def generate_payload(inputs, llm_kwargs, history, system_prompt, stream):
    ...
    if API_ORG.startswith("org-"):
        headers.update({"Cohere-Organization": API_ORG})
    ...
    if conversation_cnt:
        for index in range(0, 2 * conversation_cnt, 2):
            ...
            if what_i_have_asked["message"] != "":
                if what_gpt_answer["message"] == "":
                    continue
                if what_gpt_answer["message"] == timeout_bot_msg:
                    continue
