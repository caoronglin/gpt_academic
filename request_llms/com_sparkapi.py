...
class SparkRequestInstance:
    def __init__(self):
        XFYUN_APPID, XFYUN_API_SECRET, XFYUN_API_KEY = get_conf(
    ...
    def generate(self, inputs, llm_kwargs, history, system_prompt, use_image_api=False):
        ...
        thread.start_new_thread(
            self.create_blocking_request,
            (inputs, llm_kwargs, history, system_prompt, use_image_api),
        )
        while True:
    ...
    def create_blocking_request(
        self, inputs, llm_kwargs, history, system_prompt, use_image_api
    ):
        ...
        if use_image_api and llm_kwargs.get("most_recent_uploaded"):
            if llm_kwargs["most_recent_uploaded"].get("path"):
                file_manifest = get_pictures_list(
                    llm_kwargs["most_recent_uploaded"]["path"]
                )
    ...
    if file_manifest:
        ...
        for img_s in base64_images:
            if img_s not in str(messages):
                messages.append(
                    {"role": "user", "content": img_s, "content_type": "image"}
                )
    else:
