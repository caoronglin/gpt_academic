...
def predict_no_ui_long_connection(
    ...
    if inputs == "":
        inputs = "空空如也的输入栏"
    headers, message = generate_payload(
        inputs, llm_kwargs, history, sys_prompt, image_paths=None
    )
    retry = 0
...
def predict(
    ...
    if (
        any([llm_kwargs["llm_model"] == model for model in Claude_3_Models])
        and have_recent_file
    ):
