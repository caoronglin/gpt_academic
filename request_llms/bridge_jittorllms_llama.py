...
class GetGLMHandle(Process):
    ...
    def check_dependency(self):
        try:
            ...
            self.info = (
                r"缺少jittorllms的依赖，如果要使用jittorllms，除了基础的pip依赖以外，您还需要运行`pip install -r request_llms/requirements_jittorllms.txt -i https://pypi.jittor.org/simple -I`"
                ...
                + r"警告：安装jittorllms依赖后将完全破坏现有的pytorch环境，建议使用docker环境！"
                + trimmed_format_exc()
            )
            self.success = False
        ...
        def load_model():
            ...
            try:
                if self.jittorllms_model is None:
                    ...
                    self.jittorllms_model = get_model(
                        types.SimpleNamespace(**args_dict)
                    )
                    print("done get model")
            except:
                self.child.send(
                    "[Local Message] Call jittorllms fail 不能正常加载jittorllms的参数。"
                )
...
def predict_no_ui_long_connection(
    ...
    response = ""
    for response in llama_glm_handle.stream_chat(
        query=inputs,
