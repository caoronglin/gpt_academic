...
class ZhipuChatInit:
    ...
    def __conversation_user(self, user_input: str, llm_kwargs: dict):
        if self.model not in ["glm-4v"]:
            ...
            if encode_img:
                ...
                img_d = {
                    "type": "image_url",
                    "image_url": {"url": encode_img[0]["data"]},
                }
    ...
    def __conversation_message_payload(
        self, inputs: str, llm_kwargs: dict, history: list, system_prompt: str
    ):
        messages = []
        if system_prompt:
        ...
        if (
            inputs.strip() == ""
        ):  # 处理空输入导致报错的问题 https://github.com/binary-husky/gpt_academic/issues/1640 提示 {"error":{"code":"1214","message":"messages[1]:content和tool_calls 字段不能同时为空"}
    ...
    def generate_chat(
        ...
        self.model = llm_kwargs["llm_model"]
        response = self.__conversation_message_payload(
