...
def 注释源代码(
    ...
    for index, fp in enumerate(file_manifest):
        ...
        i_say = (
            prefix
            + f"Please conclude the following source code at {os.path.relpath(fp, project_folder)} with only one sentence{lang}, the code is:\n```{file_content}```"
        )
        i_say_show_user = (
            prefix
        ...
        i_say, _ = input_clipping(
            inputs=i_say, history=[], max_token_limit=MAX_TOKEN_SINGLE_FILE
        )
        inputs_array.append(i_say)
        inputs_show_user_array.append(i_say_show_user)
        history_array.append([])
        sys_prompt_array.append(
            f"You are a software architecture analyst analyzing a source code project. Do not dig into details, tell me what the code is doing in general. Your answer must be short, simple and clear{lang}."
        )
    ...
    def bark_fn(tasks):
        for i in range(len(tasks)):
            tasks[i] = "watchdog is dead"

    wd = WatchDog(
        timeout=10,
        bark_fn=lambda: bark_fn(tasks),
        interval=3,
        msg="ThreadWatcher timeout",
    )
    wd.begin_watch()
    ...
    def _task_multi_threading(i_say, gpt_say, fp, file_tree_struct, index):
        ...
        pcc = PythonCodeComment(
            llm_kwargs,
            plugin_kwargs,
            language=language,
            observe_window_update=observe_window_update,
        )
        ...
        html_template = html_template.replace(
            ...
            pcc.get_markdown_block_in_html(
                markdown_convertion_for_file(warp(pcc.original_content))
            ),
        )
        html_template = html_template.replace(
            "REPLACE_CODE_FILE_RIGHT",
            pcc.get_markdown_block_in_html(
                markdown_convertion_for_file(warp(revised_content))
            ),
        )
    ...
    futures = []
    index = 0
    for i_say, gpt_say, fp in zip(
        gpt_response_collection[0::2], gpt_response_collection[1::2], file_manifest
    ):
        future = executor.submit(
            _task_multi_threading, i_say, gpt_say, fp, file_tree_struct, index
        )
        index += 1
