...
def get_video_resource(search_keyword):
    ...
    videos = search_videos(search_keyword)
...
def download_video(bvid, user_name, chatbot, history):
    ...
    yield from update_ui_latest_msg(
        f"已完成的文件: <br/>" + file_links, chatbot=chatbot, history=history, delay=0
    )
...
def 多媒体任务(
    ...
    candidate_dictionary_as_str = json.dumps(
        candidate_dictionary, ensure_ascii=False, indent=4
    )

    # 展示候选资源
    candidate_display = "\n".join(
        [f"{i+1}. {it['title']}" for i, it in enumerate(candidate_dictionary)]
    )
