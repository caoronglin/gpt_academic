...
def input_clipping(inputs, history, max_token_limit, return_clip_flags=False):
    ...
    while n_token > max_token_limit:
        ...
        everything[where] = enc.decode(clipped_encoded)[
            :-1
        ...
        while True:
            ...
            try:
                ...
                if handle_token_exceed:
                    ...
                    inputs, history = input_clipping(
                        inputs, history, max_token_limit=MAX_TOKEN - EXCEED_ALLO
                    )
                    mutable[
                        0
                    ] += f"[Local Message] 警告，文本过长将进行截断，Token溢出数：{n_exceed}。\n\n"
                    continue  # 返回重试
                else:
                    # 【选择放弃】
                    tb_str = "```\n" + trimmed_format_exc() + "```"
                    mutable[
                        0
                    ] += f"[Local Message] 警告，在执行过程中遭遇问题, Traceback：\n\n{tb_str}\n\n"
                    return mutable[0]  # 放弃
            except:
                # 【第三种情况】：其他错误：重试几次
                tb_str = "```\n" + trimmed_format_exc() + "```"
...
def can_multi_process(llm) -> bool:
    ...
    def default_condition(llm) -> bool:
        ...
        if llm.startswith("chatgpt-"):
            return True
        if llm.startswith("api2d-"):
            return True
        if llm.startswith("azure-"):
            return True
        if llm.startswith("spark"):
            return True
        if llm.startswith("zhipuai") or llm.startswith("glm-"):
            return True
        return False
...
def request_gpt_model_multi_threads_with_very_awesome_ui_and_high_efficiency(
    ...
    # 异步任务开始
    futures = [
        executor.submit(_req_gpt, index, inputs, history, sys_prompt)
        for index, inputs, history, sys_prompt in zip(
        ...
        stat_str = "".join(
            [
                (
                    ...
                    if not done
                    else f"`{mutable[thread_index][2]}`\n\n"
                )
                for thread_index, done, obs in zip(
                    range(len(worker_done)), worker_done, observe_win
                )
            ]
        )
        # 在前端打印些好玩的东西
...
def read_and_clean_pdf_text(fp):
    ...
    **函数功能**
    ...
    fb = 2  # Index 2 框框
    REMOVE_FOOT_NOTE = (
        True  # 是否丢弃掉 不是正文的内容 （比正文字体小，如参考文献、脚注、图注等）
    )
    ...
    def primary_ffsize(l):
        ...
        fsize_statistics = {}
        for wtf in l["spans"]:
            if wtf["size"] not in fsize_statistics:
                fsize_statistics[wtf["size"]] = 0
    ...
    with fitz.open(fp) as doc:
        ...
        for index, page in enumerate(doc):
            ...
            meta_font.extend(
                [
                    np.mean(
                        [
                            np.mean([wtf["size"] for wtf in l["spans"]])
                            for l in t["lines"]
                        ]
                    )
                    for t in text_areas["blocks"]
        ...
        try:
            ...
            for span in meta_span:
                if span[1] not in fsize_statistics:
                    fsize_statistics[span[1]] = 0
        ...
        for index, line in enumerate(meta_line):
            ...
            if ffsize_same(meta_line[index][fs], meta_line[index - 1][fs]):
                # 尝试识别段落
                if (
                    meta_line[index][fc].endswith(".")
                    and (meta_line[index - 1][fc] != "NEW_BLOCK")
                    and (meta_line[index][fb][2] - meta_line[index][fb][0])
                ...
                if (index + 1 < len(meta_line)) and meta_line[index][fs] > main_fsize:
                    # 单行 + 字体大
...
def get_files_from_everything(txt, type):  # type='.md'
    ...
    if txt.startswith("http"):
        ...
        path = os.path.join(
            get_log_folder(plugin_name="web_download"), gen_time_str() + type
        )
        ...
        project_folder = txt
        file_manifest = [
            f for f in glob.glob(f"{project_folder}/**/*" + type, recursive=True)
        ]
...
class nougat_interface:
    ...
    def NOUGAT_parse_pdf(self, fp, chatbot, history):
        ...
        from toolbox import gen_time_str, get_log_folder
...
def get_plugin_arg(plugin_kwargs, key, default):
    # 如果参数是空的
    if (key in plugin_kwargs) and (plugin_kwargs[key] == ""):
        plugin_kwargs.pop(key)
    # 正常情况
