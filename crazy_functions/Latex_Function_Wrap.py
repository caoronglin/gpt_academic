...
class Arxiv_Localize(GptAcademicPluginTemplate):
    ...
    def define_arg_selection_menu(self):
        ...
        gui_definition = {
            ...
            "allow_cloudio": ArgProperty(
                title="是否允许从GPTAC学术云下载(或者上传)翻译结果(仅针对Arxiv论文)",
                options=["允许", "禁止"],
                default_value="禁止",
                description="共享文献，互助互利",
                type="dropdown",
            ).model_dump_json(),
        }
    ...
    def execute(
        ...
        if allow_cache == "从头执行":
            plugin_kwargs["advanced_arg"] = (
                "--no-cache " + plugin_kwargs["advanced_arg"]
            )

        # 从云端下载翻译结果，以及上传翻译结果到云端；人人为我，我为人人。
        if allow_cloudio == "允许":
            plugin_kwargs["advanced_arg"] = (
                "--allow-cloudio " + plugin_kwargs["advanced_arg"]
            )

        yield from Latex翻译中文并重新编译PDF(
            txt,
            llm_kwargs,
            plugin_kwargs,
            chatbot,
