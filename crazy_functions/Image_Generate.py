...
def edit_image(
    llm_kwargs, prompt, image_path, resolution="1024x1024", model="dall-e-2"
):
    import json
    import os
    import time
...
@CatchException
def 图片生成_DALLE2(
    prompt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request
):
    """
    ...
    if prompt.strip() == "":
        chatbot.append(
            (
                prompt,
                "[Local Message] 图像生成提示为空白，请在“输入区”输入图像生成提示。",
            )
        )
    ...
    chatbot.append(
        (
            "您正在调用“图像生成”插件。",
            "[Local Message] 生成图像, 使用前请切换模型到GPT系列。如果中文Prompt效果不理想, 请尝试英文Prompt。正在处理中 .....",
        )
    )
...
def 图片生成_DALLE3(
    ...
    if ("advanced_arg" in plugin_kwargs) and (plugin_kwargs["advanced_arg"] == ""):
        plugin_kwargs.pop("advanced_arg")
    resolution_arg = plugin_kwargs.get(
        "advanced_arg", "1024x1024-standard-vivid"
    ).lower()
    ...
    for part in parts[1:]:
        if part in ["hd", "standard"]:
            ...
            style = part
    image_url, image_path = gen_image(
        llm_kwargs, prompt, resolution, model="dall-e-3", quality=quality, style=style
    )
    chatbot.append(
        [
            prompt,
...
class ImageEditState(GptAcademicState):
    ...
    def get_image_file(self, x):
        ...
        confirm = (
            len(file_manifest) >= 1
            and file_manifest[0].endswith(".png")
            and os.path.exists(file_manifest[0])
        )
        file = None if not confirm else file_manifest[0]
        return confirm, file

    def lock_plugin(self, chatbot):
        chatbot._cookies["lock_plugin"] = (
            "crazy_functions.Image_Generate->图片修改_DALLE2"
        )
    ...
    def get_prompt(self, x):
        confirm = (
            (len(x) >= 5)
            and (not self.get_resolution(x)[0])
            and (not self.get_image_file(x)[0])
        )
        return confirm, x
    ...
    def reset(self):
        self.req = [
            {
                "value": None,
                "description": "请先上传图像（必须是.png格式）, 然后再次点击本插件",
                "verify_fn": self.get_image_file,
            },
            {
                "value": None,
                "description": "请输入分辨率,可选：256x256, 512x512 或 1024x1024, 然后再次点击本插件",
                "verify_fn": self.get_resolution,
            },
            {
                "value": None,
                "description": "请输入修改需求,建议您使用英文提示词, 然后再次点击本插件",
...
def 图片修改_DALLE2(
    ...
    chatbot.append(
        [
            "图片修改, 执行中",
            f"图片:`{image_path}`<br/>分辨率:`{resolution}`<br/>修改需求:`{prompt}`",
        ]
    )
    yield from update_ui(chatbot=chatbot, history=history)
