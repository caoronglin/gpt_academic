...
class GenericDocumentStructureExtractor:
    ...
    def extract_document_structure(
        self, file_path: str, strategy: str = "fast"
    ) -> StructuredDocument:
        """提取文档结构
        ...
        try:
            ...
            try:
                ...
                if (
                    paper_result and len(paper_result.sections) > 2
                ):  # 如果成功识别为论文结构
            ...
            # 2. 使用通用方法提取文档结构
            elements = partition(
                str(file_path), strategy=strategy, include_metadata=True, nlp=False
            )
            ...
            return StructuredDocument(
                ...
                sections=[
                    DocumentSection(
                        title="", content="", level=0, section_type="content"
                    )
                ],
            )
    ...
    def _convert_paper_to_document(self, paper: StructuredPaper) -> StructuredDocument:
        ...
        doc = StructuredDocument(
            title=paper.metadata.title, is_paper=True, full_text=paper.full_text
        )
        ...
        doc.metadata = {
            ...
            "abstract": (
                paper.metadata.abstract if hasattr(paper.metadata, "abstract") else ""
            ),
            "is_paper": True,
        }
    ...
    def _convert_paper_sections(
        self, paper_sections: List[PaperSection], level: int = 0
    ...
    def _extract_generic_structure(self, elements) -> StructuredDocument:
        ...
        for i, element in enumerate(elements):
            ...
            # 也检查格式像标题的普通文本
                ...
                if any(re.match(pattern, text) for pattern in self.HEADING_PATTERNS):
                    # 检查长度和后续内容以确认是否为标题
                    if len(text) < 100 and self._has_sufficient_following_content(
                        i, elements
                    ):
                        is_heading = True
                        title_text = text
        ...
        for i, (index, title_text, level, section_type) in enumerate(title_elements):
            ...
            content = self._extract_content_between(
                elements, content_start, content_end
            )
        ...
        if not sections:
            ...
            default_section = DocumentSection(
                title="", content=all_content, level=0, section_type="content"
            )
    ...
    def _has_sufficient_following_content(
        self, index: int, elements, min_chars: int = 150
    ) -> bool:
        """检查元素后是否有足够的内容
    ...
    def _format_sections_markdown(
        self, sections: List[DocumentSection], base_level: int = 0
    ) -> str:
        """递归格式化章节为Markdown
        ...
        for section in sections:
            ...
            if section.subsections:
                md_parts.append(
                    self._format_sections_markdown(section.subsections, base_level)
                )
