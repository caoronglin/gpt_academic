...
class CodeSearchHandler(BaseHandler):
    ...
    async def handle(
        ...
        # 获取代码文件内容
        enhanced_code_results = await self._get_code_details(
            code_results[: search_params["max_details"]]
        )
        self.ranked_repos = [
            item["repository"] for item in enhanced_code_results if "repository" in item
        ]
