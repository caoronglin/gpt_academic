...
class BaseHandler(ABC):
    ...
    def _get_search_params(self, plugin_kwargs: Dict) -> Dict:
        """获取搜索参数"""
        return {
            "max_repos": plugin_kwargs.get(
                "max_repos", 150
            ),  # 最大仓库数量，从30改为150
            "max_details": plugin_kwargs.get(
                "max_details", 80
            ),  # 最多展示详情的仓库数量，新增参数
    ...
    async def _search_bilingual_repositories(
        self,
        english_query: str,
        chinese_query: str,
        language: str = None,
    ...
    async def _search_code(
        self, query: str, language: str = None, per_page: int = 30
    ) -> List[Dict]:
        """搜索代码"""
        try:
            # 构建查询字符串
            ...
            # 执行搜索
            result = await self.github.search_code(query=query, per_page=per_page)
    ...
    async def _search_bilingual_code(
        self,
        english_query: str,
        chinese_query: str,
        language: str = None,
        ...
        try:
            ...
            english_results = await self._search_code(
                query=english_query, language=language, per_page=per_page
            )

            # 搜索中文代码
            chinese_results = await self._search_code(
                query=chinese_query, language=language, per_page=per_page
            )
    ...
    async def _search_users(self, query: str, per_page: int = 30) -> List[Dict]:
        """搜索用户"""
        try:
            result = await self.github.search_users(query=query, per_page=per_page)
    ...
    async def _search_bilingual_users(
        self, english_query: str, chinese_query: str, per_page: int = 30
    ) -> List[Dict]:
        """同时搜索中英文用户并合并结果"""
        try:
            # 搜索英文用户
            english_results = await self._search_users(
                query=english_query, per_page=per_page
            )

            # 搜索中文用户
            chinese_results = await self._search_users(
                query=chinese_query, per_page=per_page
            )
    ...
    async def _search_topics(self, query: str, per_page: int = 30) -> List[Dict]:
        """搜索主题"""
        try:
            result = await self.github.search_topics(query=query, per_page=per_page)
    ...
    async def _search_bilingual_topics(
        self, english_query: str, chinese_query: str, per_page: int = 30
    ) -> List[Dict]:
        """同时搜索中英文主题并合并结果"""
        try:
            ...
            if merged_results and "featured" in merged_results[0]:
                merged_results.sort(
                    key=lambda x: x.get("featured", False), reverse=True
                )
            ...
            try:
                ...
                if owner and repo_name:
                    ...
                    if readme and "decoded_content" in readme:
                        ...
                        repo["readme_excerpt"] = (
                            readme["decoded_content"][:1000] + "..."
                        )

                    # 获取语言使用情况
                    languages = await self.github.get_repository_languages(
                        owner, repo_name
                    )
                    if languages:
                    ...
                    # 获取最新发布版本
                    releases = await self.github.get_repo_releases(
                        owner, repo_name, per_page=1
                    )
