...
论文分析结果："""
    ...
    def _generate_priority_summary(
        self, journal_recommendations: str, conference_recommendations: str
    ) -> Generator:
        """生成优先级总结"""
...
**注意：所有时间规划都应基于当前时间{current_date_str}进行计算，不要使用虚构的时间。**"""
    ...
    def save_recommendations(
        ...
        conference_recommendations: str,
...
"""
            ...
            result_file = write_history_to_file(
                history=[md_content], file_basename=f"期刊会议推荐_{timestamp}.md"
            )

            if result_file and os.path.exists(result_file):
                promote_file_to_downloadzone(result_file, chatbot=self.chatbot)
                self.chatbot.append(
                    ["保存成功", f"推荐报告已保存至: {os.path.basename(result_file)}"]
                )
                yield from update_ui(chatbot=self.chatbot, history=self.history)
            else:
    ...
    def recommend_venues(self, paper_path: str) -> Generator:
        ...
        conference_recommendations = (
            yield from self._generate_conference_recommendations()
        )

        # 生成优先级总结
        priority_summary = yield from self._generate_priority_summary(
            journal_recommendations, conference_recommendations
        )
        ...
        # 保存报告
        yield from self.save_recommendations(
            journal_recommendations, conference_recommendations, priority_summary
        )
...
"""
            ...
            self.chatbot.append(
                [
                    "✅ 分析完成",
                    "所有分析结果和推荐内容已添加到对话历史中，您可以继续基于这些内容提问。",
                ]
            )

        except Exception as e:
            self.chatbot.append(
                ["警告", f"添加到历史记录时出错: {str(e)}，但推荐报告已正常生成"]
            )
            # 即使添加历史失败，也不影响主要功能
...
def 论文期刊会议推荐(
    ...
    chatbot.append(
        [
            "函数插件功能及使用方式",
            "论文期刊会议推荐：基于论文内容分析，为您推荐合适的学术期刊和会议投稿目标。适用于各个学科专业（自然科学、工程技术、医学、社会科学、人文学科等），根据不同学科的评价标准和发表文化，提供分层次的期刊会议推荐、影响因子分析、发表难度评估、投稿策略建议等。<br><br>📋 使用方式：<br>1、直接上传PDF文件<br>2、输入DOI号或arXiv ID<br>3、点击插件开始分析",
        ]
    )
    ...
    # 开始推荐
    chatbot.append(
        ["开始分析", f"正在分析论文并生成期刊会议推荐: {os.path.basename(paper_file)}"]
    )
    yield from update_ui(chatbot=chatbot, history=history)

    recommender = JournalConferenceRecommender(
