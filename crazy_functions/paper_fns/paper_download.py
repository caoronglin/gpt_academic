...
def extract_paper_id(txt):
    ...
    for pattern in arxiv_patterns:
        ...
        if match:
            ...
            if re.match(r"^\d{4}\.\d{4,5}$", paper_id) or re.match(
                r"^[\w-]+/\d{7}$", paper_id
            ):
...
@CatchException
def 论文下载(
    txt: str, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request
):
    """
    ...
    chatbot.append(
        [
            f"开始下载",
            f"支持多行输入下载多篇论文，共检测到 {len(paper_infos)} 篇论文，开始下载...",
        ]
    )
    yield from update_ui(chatbot=chatbot, history=history)
    ...
    if downloaded_files:
        try:
            ...
            chatbot.append([f"创建压缩包失败", f"打包文件时出现错误: {str(e)}"])
            yield from update_ui(chatbot=chatbot, history=history)
    ...
    chatbot.append([f"下载完成", summary])
    yield from update_ui(chatbot=chatbot, history=history)

    # 如果下载成功且用户想要直接阅读内容
    if downloaded_files:
        chatbot.append(["提示", "正在读取论文内容进行分析，请稍候..."])
        yield from update_ui(chatbot=chatbot, history=history)
        ...
        # 添加提示信息
        chatbot.append(
            ["提示", "论文内容已加载完毕，您可以直接向AI提问有关该论文的问题。"]
        )
        yield from update_ui(chatbot=chatbot, history=history)
