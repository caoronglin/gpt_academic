...
class WikipediaAPI:
    ...
    async def search(
        self, query: str, limit: int = 10, namespace: int = 0
    ) -> List[Dict]:
        """
        搜索维基百科文章
    ...
    async def get_random_articles(
        self, count: int = 1, namespace: int = 0
    ) -> List[Dict]:
        """
        获取随机文章
        ...
        async with aiohttp.ClientSession(headers=self.headers) as session:
            ...
            async with session.get(self.base_url, params=params) as response:
                ...
                async with session.post(
                    self.base_url, data=login_params
                ) as login_response:
                    ...
                    if login_data.get("login", {}).get("result") == "Success":
                        ...
                        print(
                            f"登录失败: {login_data.get('login', {}).get('reason', '未知原因')}"
                        )
                        return False
    ...
    async def setup_oauth(
        ...
        try:
            ...
            if access_token and access_secret:
                ...
                # 创建消费者
                consumer = mwoauth.Consumer(consumer_token, consumer_secret)

                # 初始化握手
                redirect, request_token = mwoauth.initiate(
                    f"https://{self.language}.wikipedia.org/w/index.php", consumer
                )
...
async def example_usage():
    ...
    try:
        ...
        for i, article in enumerate(random_articles, 1):
            ...
            # 显示随机文章的简短摘要
            article_summary = await wiki_zh.get_summary(
                article.get("title"), sentences=1
            )
