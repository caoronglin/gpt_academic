...
def remove_model_prefix(llm):
    if llm.startswith("api2d-"):
        llm = llm.replace("api2d-", "")
    if llm.startswith("azure-"):
        llm = llm.replace("azure-", "")
    return llm


@CatchException
def Multi_Agent_Legacy终端(
    ...
    if model_info[llm_kwargs["llm_model"]]["max_token"] < 8000:  # 至少是8k上下文的模型
        chatbot.append(
            [
                f"处理任务: {txt}",
                f"当前插件只支持{str(supported_llms)}, 当前模型{llm_kwargs['llm_model']}的最大上下文长度太短, 不能支撑AutoGen运行。",
            ]
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    if (
        model_info[llm_kwargs["llm_model"]]["endpoint"] is not None
    ):  # 如果不是本地模型，加载API_KEY
    ...
    if persistent_class_multi_user_manager.already_alive(persistent_key):
        ...
        exit_reason = yield from executor.main_process_ui_control(
            txt, create_or_resume="resume"
        )
    else:
        ...
        chatbot.append(
            [
                "正在启动: Multi_Agent_Legacy终端",
                "插件动态生成, 执行开始, 作者 Microsoft & Binary-Husky.",
            ]
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        executor = AutoGenMath(
            llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request
        )
        persistent_class_multi_user_manager.set(persistent_key, executor)
        exit_reason = yield from executor.main_process_ui_control(
            txt, create_or_resume="create"
        )

    if exit_reason == "wait_feedback":
        # 当用户点击了“等待反馈”按钮时，将executor存储到cookie中，等待用户的再次调用
        executor.chatbot.get_cookies()[
            "lock_plugin"
        ] = "crazy_functions.Multi_Agent_Legacy->Multi_Agent_Legacy终端"
    else:
