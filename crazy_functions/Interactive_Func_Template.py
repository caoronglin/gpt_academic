...
def 交互功能模板函数(
    ...
    chatbot.append(
        (
            "这是什么功能？",
            "Interactive_Func_Template。在执行完成之后, 可以将自身的状态存储到cookie中, 等待用户的再次调用。",
        )
    )
    ...
    if state is None:
        chatbot._cookies["lock_plugin"] = (
            "crazy_functions.Interactive_Func_Template->交互功能模板函数"  # 赋予插件锁定 锁定插件回调路径，当下一次用户提交时，会直接转到该函数
        )
        ...
        chatbot.append(
            (
                "第一次调用：",
                "请输入关键词, 我将为您查找相关壁纸, 建议使用英文单词, 插件锁定中，请直接提交即可。",
            )
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    ...
    if state == "wait_user_keyword":
        ...
        inputs = inputs_show_user = (
            f"Extract all image urls in this html page, pick the first 5 images and show them with markdown format: \n\n {page_return}"
        )
