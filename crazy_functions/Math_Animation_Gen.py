...
def inspect_dependency(chatbot, history):
    ...
    try:
        ...
        chatbot.append(
            [
                "导入依赖失败",
                "使用该模块需要额外依赖，安装方法:```pip install manim manimgl```",
            ]
        )
    ...
    try:
        ...
        subprocess.check_output(
            [
                sys.executable,
                "-c",
                f"from gpt_log.MyAnimation import {class_name}; {class_name}().render()",
            ]
        )
        shutil.move(
            f"media/videos/1080p60/{class_name}.mp4",
            f"gpt_log/{class_name}-{time_str}.mp4",
        )
...
def 动画生成(
    ...
    gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
        ...
        chatbot=chatbot,
        history=demo,
        sys_prompt=r"Write a animation script with 3blue1brown's manim. "
