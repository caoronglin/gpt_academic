...
class ArgProperty(BaseModel):  # PLUGIN_ARG_MENU
    ...
    type: str = Field(
        description="The type", default=""
    )  # currently we support ['string', 'dropdown']
    options: List[str] = Field(
        default=[], description="List of options available for the argument"
...
class GptAcademicPluginTemplate:
    ...
    def define_arg_selection_menu(self):
        ...
        raise NotImplementedError(
            "You need to implement this method in your plugin class"
        )
    ...
    def get_js_code_for_generating_menu(self, btnName):
        ...
        if len(define_arg_selection.keys()) > 8:
            raise ValueError(
                "You can only have up to 8 arguments in the define_arg_selection"
            )
        ...
        return base64.b64encode(DEFINE_ARG_INPUT_INTERFACE.encode("utf-8")).decode(
            "utf-8"
        )
