...
from toolbox import (CatchException, gen_time_str, get_log_folder,
                     ...
                     trimmed_format_exc, update_ui, write_history_to_file)
...
def 多文件翻译(
    ...
    if language == "en->zh":
        inputs_array = [
            "This is a Markdown file, translate it into Chinese, do NOT modify any existing Markdown commands, do NOT use code wrapper (```), ONLY answer me with translated results:"
            + f"\n\n{frag}"
        ...
        sys_prompt_array = [
            ...
            + plugin_kwargs.get("additional_prompt", "")
            for _ in range(n_split)
        ]
    elif language == "zh->en":
        inputs_array = [
            f"This is a Markdown file, translate it into English, do NOT modify any existing Markdown commands, do NOT use code wrapper (```), ONLY answer me with translated results:"
            + f"\n\n{frag}"
            for frag in pfg.sp_file_contents
        ]
        inputs_show_user_array = [f"翻译 {f}" for f in pfg.sp_file_tag]
        ...
        inputs_array = [
            f"This is a Markdown file, translate it into {language}, do NOT modify any existing Markdown commands, do NOT use code wrapper (```), ONLY answer me with translated results:"
            + f"\n\n{frag}"
            for frag in pfg.sp_file_contents
        ]
        inputs_show_user_array = [f"翻译 {f}" for f in pfg.sp_file_tag]
    ...
    try:
        ...
        for i_say, gpt_say in zip(
            gpt_response_collection[0::2], gpt_response_collection[1::2]
        ):
    ...
    res = write_history_to_file(
        gpt_response_collection, file_basename=create_report_file_name
    )
    ...
    if txt.startswith("http"):
        ...
        if preference == "Github":
            ...
            if not txt.endswith(".md"):
                ...
                url = (
                    txt.replace("https://github.com/", "https://api.github.com/repos/")
                    + "/readme"
                )
        ...
        project_folder = txt
        file_manifest = [
            f for f in glob.glob(f"{project_folder}/**/*.md", recursive=True)
        ]
    else:
        project_folder = None
        file_manifest = []
...
def Markdown英译中(
    ...
    # 基本信息：功能、贡献者
    chatbot.append(
        ["函数插件功能？", "对整个Markdown项目进行翻译。函数插件贡献者: Binary-Husky"]
    )
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
    ...
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何.md文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
