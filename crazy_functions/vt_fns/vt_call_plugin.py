...
from toolbox import disable_auto_promotion, update_ui_latest_msg
...
def read_avail_plugin_enum():
    ...
    plugin_arr = {
        k: v for k, v in plugin_arr.items() if ("Info" in v) and ("Function" in v)
    }
    plugin_arr_info = {
        "F_{:04d}".format(i): v["Info"]
        for i, v in enumerate(plugin_arr.values(), start=1)
    }
    plugin_arr_dict = {
        "F_{:04d}".format(i): v for i, v in enumerate(plugin_arr.values(), start=1)
    }
    plugin_arr_dict_parse = {
        "F_{:04d}".format(i): v for i, v in enumerate(plugin_arr.values(), start=1)
    }
    plugin_arr_dict_parse.update(
        {f"F_{i}": v for i, v in enumerate(plugin_arr.values(), start=1)}
    )
...
def execute_plugin(
    ...
    plugin_arr_enum_prompt, plugin_arr_dict, plugin_arr_dict_parse = (
        read_avail_plugin_enum()
    )

    class Plugin(BaseModel):
        plugin_selection: str = Field(
            description="The most related plugin from one of the PluginEnum.",
            default="F_0000",
        )
        reason_of_selection: str = Field(
            description="The reason why you should select this plugin.",
    ...
    gpt_json_io.format_instructions += (
        "The plugins you are authorized to use are listed below:\n"
    )
    gpt_json_io.format_instructions += plugin_arr_enum_prompt
    inputs = (
        "Choose the correct plugin according to user requirements, the user requirement is: \n\n"
        + ">> "
        + txt.rstrip("\n").replace("\n", "\n>> ")
        + "\n\n"
        + gpt_json_io.format_instructions
    ...
    try:
        ...
        msg += "请求的Prompt为：\n" + wrap_code(
            get_inputs_show_user(inputs, plugin_arr_enum_prompt)
        )
        msg += "语言模型回复为：\n" + wrap_code(gpt_reply)
        msg += "\n但您可以尝试再试一次\n"
        yield from update_ui_latest_msg(
    ...
    gpt_json_io.format_instructions += (
        "The information about this plugin is:" + plugin["Info"]
    )
    inputs = (
        f"A plugin named {plugin_sel.plugin_selection} is selected, "
    ...
    plugin_sel = gpt_json_io.generate_output_auto_repair(
        run_gpt_fn(inputs, ""), run_gpt_fn
    )
