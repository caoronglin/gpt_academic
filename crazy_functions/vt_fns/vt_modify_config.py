...
from toolbox import get_conf, update_ui_latest_msg


def modify_configuration_hot(
    ...
    class ModifyConfigurationIntention(BaseModel):
        ...
        new_option_value: str = Field(
            description="the new value of the option", default=None
        )
    ...
    inputs = (
        "Analyze how to change configuration according to following user input, answer me with json: \n\n"
        + ">> "
        + txt.rstrip("\n").replace("\n", "\n>> ")
        + "\n\n"
    ...
    if ok:
        ...
        yield from update_ui_latest_msg(
            lastmsg=f"失败，如果需要配置{explicit_conf}，您需要明确说明并在指令中提到它。",
            chatbot=chatbot,
            history=history,
            delay=5,
        )
...
def modify_configuration_reboot(
    txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_intention
):
    ALLOW_RESET_CONFIG = get_conf("ALLOW_RESET_CONFIG")
    if not ALLOW_RESET_CONFIG:
    ...
    yield from modify_configuration_hot(
        txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_intention
    )
    yield from update_ui_latest_msg(
        lastmsg=f"正在执行任务: {txt}\n\n配置修改完成，五秒后即将重启！若出现报错请无视即可。",
        chatbot=chatbot,
        history=history,
        delay=5,
    )
