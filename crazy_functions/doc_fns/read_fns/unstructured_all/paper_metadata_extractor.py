...
class PaperMetadataExtractor:
    ...
    def _extract_title_and_authors(self, elements, metadata: PaperMetadata) -> None:
        ...
        for i, text in enumerate(all_text[:15]):
            ...
            if re.match(r"^[A-Z][A-Z0-9\s\.]+(\s+K\d+(\.\d+)?)?:", text):
                ...
                if (
                    i + 1 < len(all_text)
                    and all_text[i + 1].isupper()
                    and len(all_text[i + 1]) > 10
                ):
        ...
        if title_candidates:
            ...
            for text in all_text[:10]:
                if (
                    text.isupper() and len(text) > 10 and len(text) < 200
                ):  # 大写且适当长度的文本
                    metadata.title = text
                    break
            title_position = 0
        ...
        if not metadata.authors:
            for text in all_text[:20]:
                if re.search(
                    r"\b(team|group|lab|laboratory|研究组|团队)\b", text, re.IGNORECASE
                ):
        ...
        for i, element in enumerate(elements[:30]):
            ...
            if re.search(
                r"(university|institute|department|school|laboratory|college|center|centre|\d{5,}|^[a-zA-Z]+@|学院|大学|研究所|研究院)",
                element_text,
                re.IGNORECASE,
            ):
                # 可能是隶属机构
    ...
    def _evaluate_title_candidate(self, text, position, element):
        ...
        # 内容因素
        if re.search(
            r"\b(scaling|learning|model|approach|method|system|framework|analysis)\b",
            text.lower(),
        ):
            ...
            try:
                ...
                try:
                    ...
                    if "font_size" in metadata_dict and metadata_dict["font_size"] > 14:
                        score += 3
                    if (
                        "font_weight" in metadata_dict
                        and metadata_dict["font_weight"] == "bold"
                    ):
                        score += 2
                except Exception:
                    # 如果所有尝试都失败，忽略元数据处理
    ...
    def _extract_abstract_and_keywords(self, elements, metadata: PaperMetadata) -> None:
        ...
        for i, element in enumerate(elements):
            ...
            if not abstract_found and (
                ...
                and re.search(
                    self.SECTION_PATTERNS["abstract"], element_text, re.IGNORECASE
                )
            ):
                abstract_found = True
                continue
            ...
            if abstract_found and not keywords_found:
                ...
                if (
                    isinstance(element, Title)
                    or re.search(
                        self.SECTION_PATTERNS["keywords"], element_text, re.IGNORECASE
                    )
                    or re.match(
                        r"\b(introduction|引言|method|方法)\b",
                        element_text,
                        re.IGNORECASE,
                    )
                ):
                    keywords_found = re.search(
                        self.SECTION_PATTERNS["keywords"], element_text, re.IGNORECASE
                    )
            ...
            if keywords_found and not abstract_found and not metadata.keywords:
                if isinstance(element, (Text, NarrativeText)):
                    ...
                    cleaned_text = re.sub(
                        r"^\s*(关键词|keywords|key\s+words)\s*[：:]\s*",
                        "",
                        element_text,
                        flags=re.IGNORECASE,
                    )
                    ...
                    for separator in [";", "；", ",", "，"]:
                        if separator in cleaned_text:
                            metadata.keywords = [
                                k.strip()
                                for k in cleaned_text.split(separator)
                                if k.strip()
                            ]
                            break
    ...
    def _extract_additional_metadata(self, elements, metadata: PaperMetadata) -> None:
        ...
        for element in elements[:30]:  # 只检查文档前部分
            ...
            # 尝试匹配DOI
            doi_match = re.search(
                r"(doi|DOI):\s*(10\.\d{4,}\/[a-zA-Z0-9.-]+)", element_text
            )
            ...
            date_match = re.search(
                r"(published|received|accepted|submitted):\s*(\d{1,2}\s+[a-zA-Z]+\s+\d{4}|\d{4}[-/]\d{1,2}[-/]\d{1,2})",
                element_text,
                re.IGNORECASE,
            )
            ...
            journal_match = re.search(
                r"(journal|conference):\s*([^,;.]+)", element_text, re.IGNORECASE
            )
            if journal_match:
