...
def gpt_interact_multi_step(txt, file_type, llm_kwargs, chatbot, history):
    ...
    i_say = "".join(prompt_compose)
    inputs_show_user = "If previous stage is successful, rewrite the function you have just written to satisfy executable template. "
    gpt_say = yield from request_gpt_model_in_new_thread_with_ui_alive(
        inputs=i_say,
...
def Dynamic_Function_Generate(
    ...
    # 基本信息：功能、贡献者
    chatbot.append(
        ["正在启动: 插件动态生成插件", "插件动态生成, 执行开始, 作者Binary-Husky."]
    )
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
    ...
    if get_plugin_arg(plugin_kwargs, key="file_path_arg", default=False):
        ...
        yield from update_ui_latest_msg(
            f"当前文件处理列表: {file_list}", chatbot, history, 1
        )
    else:
        chatbot.append(["文件检索", "没有发现任何近期上传的文件。"])
        yield from update_ui_latest_msg(
            "没有发现任何近期上传的文件。", chatbot, history, 1
        )
        return  # 2. 如果没有文件
    if len(file_list) == 0:
        chatbot.append(["文件检索", "没有发现任何近期上传的文件。"])
    ...
    if is_the_upload_folder(txt):
        yield from update_ui_latest_msg(
            f"请在输入框内填写需求, 然后再次点击该插件! 至于您的文件，不用担心, 文件路径 {txt} 已经被记忆. ",
            chatbot,
            history,
            1,
        )
    ...
    for j in range(MAX_TRY):  # 最多重试5次
        ...
        try:
            ...
            code, installation_advance, txt, file_type, llm_kwargs, chatbot, history = (
                yield from gpt_interact_multi_step(
                    txt, file_type, llm_kwargs, chatbot, history
                )
            )
            chatbot.append(["代码生成阶段结束", ""])
            yield from update_ui_latest_msg(
                f"正在验证上述代码的有效性 ...", chatbot, history, 1
            )
            # ⭐ 分离代码块
    ...
    TIME_LIMIT = 15
    yield from update_ui_latest_msg(
        f"开始创建新进程并执行代码! 时间限制 {TIME_LIMIT} 秒. 请等待任务完成... ",
        chatbot,
        history,
        1,
    )
    ...
    for file_path in file_list:
        if os.path.exists(file_path):
            ...
            chatbot = for_immediate_show_off_when_possible(
                file_type, file_path, chatbot
            )
        ...
        p = multiprocessing.Process(
            target=subprocess_worker, args=(code, file_path, return_dict)
        )
        # ⭐ 开始执行，时间限制TIME_LIMIT
        p.start()
        ...
        if os.path.exists(res):
            ...
            chatbot = for_immediate_show_off_when_possible(
                file_type, new_file_path, chatbot
            )
            yield from update_ui(
                chatbot=chatbot, history=history
            )  # 刷新界面 # 界面更新
