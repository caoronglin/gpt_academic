            ...
            if text is None:
                chatbot.append(
                    [
                        ...
                        f"文件解析失败，无法提取文本内容，请更换文件。失败原因可能为：1.文档格式过于复杂；2. 不支持的文件格式，支持的文件格式后缀有:"
                        + ", ".join(supports_format),
                    ]
                )
            else:
                chatbot.append(
...
def Rag问答(
    ...
    if os.path.exists(txt) and os.path.isdir(txt):
        ...
        yield from handle_document_upload(
            file_paths,
            llm_kwargs,
            plugin_kwargs,
            chatbot,
    ...
    txt_clip, history, flags = input_clipping(
        txt, history, max_token_limit=MAX_CONTEXT_TOKEN_LIMIT, return_clip_flags=True
    )
    ...
    if len(chatbot) != 0:
        chatbot.pop(
            -1
        )  # Pop temp chat, because we are going to add them again inside `request_gpt_model_in_new_thread_with_ui_alive`
