...
def 批量翻译PDF文档(
    ...
    try:
        ...
        chatbot.append(
            [
                None,
                f"导入软件依赖失败。使用该模块需要额外依赖，安装方法```pip install --upgrade pymupdf tiktoken scipdf_parser```。",
            ]
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
    ...
    if (not success) and txt == "":
        txt = "空空如也的输入栏。提示：请先上传文件（把PDF文件拖入对话）。"
        ...
        if len(DOC2X_API_KEY) != 0:
            try:
                ...
                chatbot.append(
                    [
                        None,
                        f"DOC2X服务不可用，请检查报错详细。{trimmed_format_exc_markdown()}",
                    ]
                )
    ...
    if method == "Classic":
        ...
        yield from update_ui_latest_msg(
            "GROBID服务不可用，请检查config中的GROBID_URL。作为替代，现在将执行效果稍差的旧版代码。",
            chatbot,
            history,
            delay=3,
        )
        yield from 解析PDF_简单拆解(
            file_manifest,
            project_folder,
            llm_kwargs,
            plugin_kwargs,
        ...
        if len(DOC2X_API_KEY) != 0:
            try:
                ...
                chatbot.append(
                    [
                        None,
                        f"DOC2X服务不可用，正在尝试GROBID。{trimmed_format_exc_markdown()}",
                    ]
                )
                yield from update_ui(chatbot=chatbot, history=history)
        grobid_url = get_avail_grobid_url()
