...
def write_chat_to_file_legacy(chatbot, history=None, file_name=None):
    ...
    if file_name is None:
        file_name = (
            f_prefix + time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + ".html"
        )
    fp = os.path.join(
        get_log_folder(get_user(chatbot), plugin_name="chat_history"), file_name
    )
    ...
    with open(fp, "w", encoding="utf8") as f:
        ...
        html_content = form.format(
            CHAT_PREVIEW=CHAT_PREVIEW_BUF,
            HISTORY_PREVIEW=HISTORY_PREVIEW_BUF,
            CSS=advanced_css,
        )
...
def write_chat_to_file(chatbot, history=None, file_name=None):
    ...
    async def main():
        ...
        # 收集所有成功生成的文件
        result_files = [
            f for f in [html_file, docx_file, md_file, txt_file, pdf_file] if f
        ]
    ...
    for file in result_files:
        promote_file_to_downloadzone(
            file, rename_file=os.path.basename(file), chatbot=chatbot
        )
...
def read_file_to_chat(chatbot, history, file_name):
    ...
    chatbot.append(
        [
            None,
            f"[Local Message] 载入对话{len(qa_box_list)}条，上下文{len(history)}条。",
        ]
    )
...
def 对话历史存档(
    ...
    chatbot.append(
        (
            None,
            f"[Local Message] {write_chat_to_file_legacy(chatbot, history, file_name)}，您可以调用下拉菜单中的“载入对话历史存档”还原当下的对话。",
        )
    )
    try:
        chatbot.append(
            (None, f"[Local Message] 正在尝试生成pdf以及word格式的对话存档，请稍等...")
        )
        yield from update_ui(
            chatbot=chatbot, history=history
        )  # 刷新界面 # 由于请求需要一段时间，我们先及时地做一次界面更新
        lastmsg = (
            f"[Local Message] {write_chat_to_file(chatbot, history, file_name)}。"
        ...
        yield from update_ui_latest_msg(
            lastmsg, chatbot, history
        )  # 刷新界面 # 由于请求需要一段时间，我们先及时地做一次界面更新
    except Exception as e:
        logger.exception(
            f"已完成对话存档（pdf和word格式的对话存档生成未成功）。{str(e)}"
        )
...
class Conversation_To_File_Wrap(GptAcademicPluginTemplate):
    ...
    def define_arg_selection_menu(self):
        ...
        """
        gui_definition = {
            "file_name": ArgProperty(
                title="保存文件名",
                description="输入对话存档文件名，留空则使用时间作为文件名",
                default_value="",
                type="string",
...
def 载入对话历史存档(
    ...
    if not success:
        ...
        chatbot.append(
            [
                f"正在查找对话历史文件（html格式）: {txt}",
                f"找不到任何html文件: {txt}。但本地存储了以下历史文件，您可以将任意一个文件路径粘贴到输入区，然后重试：<br/>{local_history}",
            ]
        )
