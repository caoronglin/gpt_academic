...
class SaveAndLoad:
    ...
    def does_checkpoint_exist(self, checkpoint_dir=None):
        ...
        if not os.path.exists(checkpoint_dir):
            return False
        if len(glob.glob(os.path.join(checkpoint_dir, "social_network.pkl"))) == 0:
            return False
        return True
...
class Friend(BaseModel):
    ...
    friend_description: str = Field(
        description="description of a friend (everything about this friend)"
    )
    friend_relationship: str = Field(
        description="The relationship with a friend (e.g. friend, family, colleague)"
    )
...
class SocialNetworkWorker(SaveAndLoad):
    ...
    def ai_add_multi_friends(
        ...
        if friend.friends_list:
            ...
            yield from update_ui_latest_msg(
                lastmsg=msg, chatbot=chatbot, history=history, delay=0
            )
    ...
    def run(
        ...
        try:
            ...
            class UserSociaIntention(BaseModel):
                intention_type: str = Field(
                    description=f"The type of user intention. You must choose from {self.tools_to_select.keys()}.\n\n"
                    f"Explanation:\n{Explanation}",
                    default="SocialAdvice",
                )

            pydantic_cls_instance, err_msg = select_tool(
                prompt=txt, run_gpt_fn=run_gpt_fn, pydantic_cls=UserSociaIntention
            )
        ...
        intention_callback = self.tools_to_select[pydantic_cls_instance.intention_type][
            "callback"
        ]
        yield from intention_callback(
            txt,
            llm_kwargs,
            plugin_kwargs,
            chatbot,
...
def I人助手(
    ...
    # 2. save
    yield from social_network_worker.run(
        txt, llm_kwargs, plugin_kwargs, chatbot, history, system_prompt, user_request
    )
