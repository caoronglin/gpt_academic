...
def split_audio_file(filename, split_duration=1000):
    ...
    os.makedirs(
        f"{get_log_folder(plugin_name='audio')}/mp3/cut/", exist_ok=True
    )  # 创建存储切割音频的文件夹
    ...
    for i in range(len(split_points) - 1):
        ...
        split_audio.write_audiofile(
            f"{get_log_folder(plugin_name='audio')}/mp3/cut/{filename[0]}_{i}.mp3"
        )
        filelist.append(
            f"{get_log_folder(plugin_name='audio')}/mp3/cut/{filename[0]}_{i}.mp3"
        )
    ...
    for index, fp in enumerate(file_manifest):
        ...
        if ext not in [".mp3", ".wav", ".m4a", ".mpga"]:
            ...
            audio_clip.write_audiofile(
                f"{get_log_folder(plugin_name='audio')}/mp3/output{index}.mp3"
            )
        ...
        for j, i in enumerate(voice):
            ...
            chatbot.append(
                [
                    f"将 {i} 发送到openai音频解析终端 (whisper)，当前参数：{parse_prompt}",
                    "正在处理 ...",
                ]
            )
            ...
            response = requests.post(
                url, headers=headers, files=files, data=data, proxies=proxies
            ).text
...
def Audio_Summary(
    ...
    # 基本信息：功能、贡献者
    chatbot.append(
        ["函数插件功能？", "Audio_Summary内容，函数插件贡献者: dalvqw & BinaryHusky"]
    )
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
    ...
    if txt.endswith(tuple(extensions)):
        ...
        for extension in extensions:
            file_manifest.extend(
                glob.glob(f"{project_folder}/**/*{extension}", recursive=True)
            )

    # 如果没找到任何文件
    if len(file_manifest) == 0:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到任何音频或视频文件: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return
