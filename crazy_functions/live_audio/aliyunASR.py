...
def is_speaker_speaking(vad, data, sample_rate):
    ...
    )  # x2 because audio is 16 bit (2 bytes)
    res_list = []
...
class AliyunASR:
    ...
    def audio_convertion_thread(self, uuid):
        ...
        from .audio_io import RealtimeAudioDistribution, change_sample_rate
        ...
        while not self.stop:
            ...
            if audio is not None:
                ...
                dsdata = change_sample_rate(
                    audio, rad.rate, NEW_SAMPLERATE
                )  # 48000 --> 16000
                ...
                if is_speaking or echo_cnt > 0:
                    ...
                    echo_cnt -= 1
                    if (
                        not is_previous_frame_transmitted
                    ):  # 上一帧没有人声，但是我们把上一帧同样加上
                        if previous_frame_data is not None:
                            data = previous_frame_data + data
                    if is_speaking:
                    ...
                    if (
                        time.time() - keep_alive_last_send_time
                        > timeout_limit_second / 2
                    ):
                        slices = zip(*(iter(data),) * 640)  # 640个字节为一组
                        for i in slices:
                            sr.send_audio(bytes(i))
    ...
    def get_token(self):
        ...
        # 创建AcsClient实例
        client = AcsClient(AccessKey_ID, AccessKey_secret, "cn-shanghai")
