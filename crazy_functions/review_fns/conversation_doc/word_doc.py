...
class WordFormatter:
    ...
    def _setup_document(self):
        ...
        for section in sections:
            ...
            header_run = header_para.add_run(
                "GPT-Academic学术对话 (体验地址：https://auth.gpt-academic.top/)"
            )
    ...
    def _create_styles(self):
        ...
        # 创建问题样式
        question_style = self.doc.styles.add_style(
            "Question_Style", WD_STYLE_TYPE.PARAGRAPH
        )
        ...
        question_style.paragraph_format.line_spacing_rule = (
            WD_LINE_SPACING.ONE_POINT_FIVE
        )
        question_style.paragraph_format.left_indent = Pt(0)  # 移除左缩进

        # 创建回答样式
        answer_style = self.doc.styles.add_style(
            "Answer_Style", WD_STYLE_TYPE.PARAGRAPH
        )
        ...
        # 添加参考文献样式
        ref_style = self.doc.styles.add_style(
            "Reference_Style", WD_STYLE_TYPE.PARAGRAPH
        )
        ...
        # 添加参考文献标题样式
        ref_title_style = self.doc.styles.add_style(
            "Reference_Title_Style", WD_STYLE_TYPE.PARAGRAPH
        )
        ...
        ref_title_style.paragraph_format.line_spacing_rule = (
            WD_LINE_SPACING.ONE_POINT_FIVE
        )
                    ...
                    for idx, paper in enumerate(ranked_papers, 1):
                        try:
                            ...
                            if hasattr(paper, "url") and paper.url:
                                try:
                                    title_run._element.rPr.rStyle = (
                                        self._create_hyperlink_style()
                                    )
                                    self._add_hyperlink(
                                        ref_para, paper.title, paper.url
                                    )
                                except Exception as e:
                            ...
                            if paper.doi:
                                ...
                                self._add_hyperlink(
                                    ref_para, f"DOI: {paper.doi}", doi_url
                                )
