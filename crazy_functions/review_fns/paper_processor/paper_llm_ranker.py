...
class PaperLLMRanker:
    ...
    def _update_paper_metrics(self, papers: List[PaperMetadata]) -> None:
        ...
        for paper in papers:
            ...
            if hasattr(paper, "venue_name") or hasattr(paper, "venue_info"):
                ...
                # 使用改进的匹配逻辑获取指标
                metrics = self.journal_metrics.get_journal_metrics(
    ...
    def rank_papers(
        ...
        if len(papers) <= SMALL_PAPER_THRESHOLD:
            ...
            if search_criteria:
                if search_criteria.query_type == "latest":
                    ...
                    papers.sort(
                        key=lambda x: getattr(x, "citations", 0) or 0, reverse=True
                    )
        ...
        if search_criteria and search_criteria.query_type == "latest":
            papers = sorted(
                papers, key=lambda x: self._get_year_as_int(x), reverse=True
            )
        ...
        if len(papers) > max_papers:
            ...
            papers = self._select_papers_strategically(
                papers, search_criteria, max_papers
            )
        ...
        try:
            ...
            for paper in papers:
                ...
                if search_criteria and search_criteria.start_year:
                    if (
                        getattr(paper, "year", 0)
                        and self._get_year_as_int(paper.year)
            ...
            if search_criteria:
                if search_criteria.query_type == "latest":
                    ...
                    # 默认按IF指数排序
                    relevant_papers.sort(
                        key=lambda x: getattr(x, "if_factor", 0) or 0, reverse=True
                    )
    ...
    def _build_enhanced_query(self, query: str, criteria: SearchCriteria) -> str:
        ...
        components.append(
            f"Original user query that must be primarily matched: {query}"
        )
        ...
        if criteria:
            ...
            if criteria.main_topic and criteria.main_topic != query:
                components.append(
                    f"Additional context - The main topic is about: {criteria.main_topic}"
                )

            # 添加子主题
            if criteria.sub_topics:
                components.append(
                    f"Secondary aspects to consider: {', '.join(criteria.sub_topics)}"
                )

            # 添加查询类型相关信息
            if criteria.query_type == "review":
                components.append(
                    "Paper type preference: Looking for comprehensive review papers, survey papers, or overview papers"
                )
            elif criteria.query_type == "latest":
                components.append(
                    "Temporal preference: Focus on the most recent developments and latest papers"
                )
            elif criteria.query_type == "recommend":
                components.append(
                    "Impact preference: Consider influential and fundamental papers"
                )
    ...
    def _build_enhanced_document(
        self, paper: PaperMetadata, criteria: SearchCriteria
    ) -> str:
        """构建增强的文档表示"""
        components = []
        ...
        if criteria:
            if criteria.query_type == "review":
                ...
                if criteria.main_topic:
                    ...
                    abstract_lower = (abstract or "").lower()
                    topic_relevance = any(
                        topic.lower() in title_lower or topic.lower() in abstract_lower
                        for topic in [criteria.main_topic] + (criteria.sub_topics or [])
                    )
                    if topic_relevance:
                        components.append(
                            f"This paper is directly related to {criteria.main_topic}"
                        )
    ...
    def _select_papers_strategically(
        ...
        def calculate_paper_score(paper):
            ...
            if query_type == "review":
                ...
                if year and start_year:
                    year_int = (
                        year if isinstance(year, int) else self._get_year_as_int(paper)
                    )
                    start_year_int = (
                        start_year if isinstance(start_year, int) else int(start_year)
                    )
        ...
        # 4. 对非Crossref论文评分和排序
        scored_non_crossref = [
            (p, calculate_paper_score(p)) for p in non_crossref_papers
        ]
        ...
        if len(scored_non_crossref) >= non_crossref_limit:
            result.extend(
                [p[0] for p in scored_non_crossref[: int(non_crossref_limit)]]
            )
        else:
        ...
        if remaining_slots > 0 and papers_by_source["crossref"]:
            ...
            # 对Crossref论文评分和排序
            scored_crossref = [
                (p, calculate_paper_score(p)) for p in papers_by_source["crossref"]
            ]
