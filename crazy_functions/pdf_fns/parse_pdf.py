...
def parse_pdf(pdf_path, grobid_url):
    ...
    try:
        ...
        raise GROBID_OFFLINE_EXCEPTION(
...
def produce_report_markdown(
    ...
    # -=-=-=-=-=-=-=-= 写出第1个文件：翻译前后混合 -=-=-=-=-=-=-=-=
    res_path = write_history_to_file(
        meta + ["# Meta Translation", paper_meta_info] + gpt_response_collection,
        file_basename=f"{gen_time_str()}translated_and_original.md",
        file_fullname=None,
    )
    promote_file_to_downloadzone(
        res_path, rename_file=os.path.basename(res_path) + ".md", chatbot=chatbot
    )
    ...
    for index, value in enumerate(gpt_response_collection):
        ...
        if index % 2 != 0:
            # 先提取当前英文标题：
            cur_section_name = (
                gpt_response_collection[index - 1].split("\n")[0].split(" Part")[0]
            )
    ...
    res_path = write_history_to_file(
        meta + ["# Meta Translation", paper_meta_info] + translated_res_array,
...
def translate_pdf(
    ...
    def break_down(txt):
        ...
        if raw_token_num <= TOKEN_LIMIT_PER_FRAGMENT:
            ...
            return breakdown_text_to_satisfy_token_limit(
                txt, limit=token_limit_smooth, llm_model=llm_kwargs["llm_model"]
            )
    ...
    for i, k in enumerate(gpt_response_collection_html):
        if i % 2 == 0:
            ...
            # 先提取当前英文标题：
            cur_section_name = (
                gpt_response_collection[i - 1].split("\n")[0].split(" Part")[0]
            )
