...
def cut(limit, get_token_fn, txt_tocut, must_break_at_empty_line, break_anyway=False):
    ...
    remain_txt_to_cut_storage = ""
    # 为了加速计算，我们采样一个特殊的手段。当 remain_txt_to_cut > `_max` 时， 我们把 _max 后的文字转存至 remain_txt_to_cut_storage
    remain_txt_to_cut, remain_txt_to_cut_storage = maintain_storage(
...
breakdown_text_to_satisfy_token_limit = run_in_subprocess_with_timeout(
...
def cut_new(
    ...
    must_break_at_one_empty_line=False,
    break_anyway=False,
):
    """文本切分"""
    res = []
...
def breakdown_text_to_satisfy_token_limit_new_(txt, limit, llm_model="gpt-3.5-turbo"):
    ...
    try:
        # 第1次尝试，将双空行（\n\n）作为切分点
        res, empty_line = cut_new(
            limit, get_token_fn, txt, must_break_at_empty_line=True
        )
        ...
        try:
            ...
            res, _ = cut_new(
                ...
                txt,
                must_break_at_empty_line=False,
                must_break_at_one_empty_line=True,
            )
            return res, _
        except RuntimeError:
            try:
...
breakdown_text_to_satisfy_token_limit_new = run_in_subprocess_with_timeout(
    breakdown_text_to_satisfy_token_limit_new_, timeout=60
)


def cut_from_end_to_satisfy_token_limit_(
    txt, limit, reserve_token=500, llm_model="gpt-3.5-turbo"
):
    """从后往前裁剪文本，以论文为单位进行裁剪
...
# 添加超时保护
cut_from_end_to_satisfy_token_limit = run_in_subprocess_with_timeout(
    cut_from_end_to_satisfy_token_limit_, timeout=20
)
