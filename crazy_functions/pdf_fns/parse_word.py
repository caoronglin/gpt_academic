...
def extract_text_from_files(txt, chatbot, history):
    ...
    if file_pdf:
        ...
        for index, fp in enumerate(pdf_manifest):
            file_content, pdf_one = read_and_clean_pdf_text(
                fp
            )  # （尝试）按照章节切割PDF
            ...
            pdf_one = (
                str(pdf_one).encode("utf-8", "ignore").decode()
    ...
    if file_md:
        for index, fp in enumerate(md_manifest):
            ...
            headers = re.findall(
                r"^#\s(.*)$", file_content, re.MULTILINE
            )  # 接下来提取md中的一级/二级标题作为摘要
            if len(headers) > 0:
