...
class BatchDocumentSummarizer:
    ...
    def _create_batch_inputs(
        self, fragments: List[FileFragment]
        ...
        for frag in fragments:
            if self.query:
                i_say = (
                    f"请按照用户要求对文件内容进行处理，文件名为{os.path.basename(frag.file_path)}，"
                    f"用户要求为：{self.query}："
                ...
                i_say = (
                    f"请对下面的内容用中文做总结，不超过500字，文件名是{os.path.basename(frag.file_path)}，"
                    f"内容是 ```{frag.content}```"
                )
                i_say_show_user = f"正在处理 {frag.rel_path} (片段 {frag.fragment_index + 1}/{frag.total_fragments})"
    ...
    def _process_single_file_with_timeout(
        self, file_info: Tuple[str, str], mutable_status: List
    ) -> List[FileFragment]:
        """包装了超时控制的文件处理函数"""
        ...
        try:
            ...
            def check_timeout():
                if hasattr(thread, "_timeout_occurred") and thread._timeout_occurred:
                    raise TimeoutError(
                        f"处理文件 {os.path.basename(fp)} 超时（{TIMEOUT_SECONDS}秒）"
                    )
            ...
            if os.path.getsize(fp) > self.max_file_size:
                self.failed_files.append(
                    (fp, f"文件过大：超过{self.max_file_size / 1024 / 1024}MB")
                )
    ...
    def prepare_fragments(
        self, project_folder: str, file_paths: List[str]
    ) -> Generator:
        ...
        mutable_status_array = [
            ["等待中", time.time(), "pending", file_path] for file_path in file_paths
        ]
        ...
        try:
            ...
            while True:
                ...
                # 更新状态显示
                status_str = ""
                for i, (status, timestamp, desc, file_path) in enumerate(
                    mutable_status_array
                ):
                    # 获取文件名（去掉路径）
        ...
        for future in futures:
            try:
                ...
                file_index = futures.index(future)
                self.failed_files.append(
                    (file_paths[file_index], f"未知错误：{str(e)}")
                )
                continue
            ...
            for retry in range(max_retries):
                try:
                    ...
                    if retry == max_retries - 1:  # 最后一次重试失败
                        for frag in batch:
                            self.failed_files.append(
                                (frag.file_path, f"处理失败：{str(e)}")
                            )
                    else:
                        yield from update_ui(
                            self.chatbot.append(
                                [f"批次处理失败，{retry_delay}秒后重试...", str(e)]
                            )
                        )
        ...
        if total_files == 1:
            ...
            try:
                ...
                for rel_path, summary in self.file_summaries_map.items():
                    file_summaries_for_final.append(
                        f"文件 {rel_path} 的总结：\n{summary}"
                    )

                if self.plugin_kwargs.get("advanced_arg"):
                    final_summary_prompt = (
                        "根据以下所有文件的总结内容，按要求进行综合处理："
                        + self.plugin_kwargs["advanced_arg"]
                    )
                else:
                    final_summary_prompt = (
                        "请根据以下所有文件的总结内容，生成最终的总结报告。"
                    )
    ...
    def save_results(self, final_summary: str):
        ...
        md_formatter = MarkdownFormatter(
            final_summary, self.file_summaries_map, self.failed_files
        )
        ...
        try:
            ...
            result_file_html = write_history_to_file(
                history=[html_content], file_basename=f"文档总结_{timestamp}.html"
            )
        ...
        try:
            ...
            result_file_docx = os.path.join(
                os.path.dirname(result_file_md), f"文档总结_{timestamp}.docx"
            )
...
def 批量文件询问(
    ...
    summarizer = BatchDocumentSummarizer(
        llm_kwargs, query, chatbot, history, system_prompt
    )
    chatbot.append(
        [
            "函数插件功能",
            f"作者：lbykkkk，批量总结文件。支持格式: {', '.join(supports_format)}等其他文本格式文件，如果长时间卡在文件处理过程，请查看处理进度，然后删除所有处于“pending”状态的文件，然后重新上传处理。",
        ]
    )
    ...
    if not os.path.exists(txt):
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b=f"找不到项目或无权访问: {txt}"
        )
        yield from update_ui(chatbot=chatbot, history=history)
        return
    ...
    if not file_manifest:
        report_exception(
            chatbot, history, a=f"解析项目: {txt}", b="未找到支持的文件类型"
        )
        yield from update_ui(chatbot=chatbot, history=history)
        return
