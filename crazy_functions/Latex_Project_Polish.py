...
from toolbox import (CatchException, get_log_folder,
...
def 多文件润色(
    ...
    if language == "en":
        if mode == "polish":
            inputs_array = [
                r"Below is a section from an academic paper, polish this section to meet the academic standard, "
        ...
        if mode == "polish":
            inputs_array = [
                r"以下是一篇学术论文中的一段内容，请将此部分润色以满足学术标准，提高语法、清晰度和整体可读性，不要修改任何LaTeX命令，例如\section，\cite和方程式："
                + f"\n\n{frag}"
                for frag in pfg.sp_file_contents
            ]
        else:
            inputs_array = [
                r"以下是一篇学术论文中的一段内容，请对这部分内容进行语法矫正。不要修改任何LaTeX命令，例如\section，\cite和方程式："
                + f"\n\n{frag}"
                for frag in pfg.sp_file_contents
            ]
    ...
    create_report_file_name = (
        time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime()) + f"-chatgpt.polish.md"
    )
...
def Latex中文润色(
    ...
    # 基本信息：功能、贡献者
    chatbot.append(
        ["函数插件功能？", "对整个Latex项目进行润色。函数插件贡献者: Binary-Husky"]
    )
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
...
def Latex英文纠错(
    ...
    # 基本信息：功能、贡献者
    chatbot.append(
        ["函数插件功能？", "对整个Latex项目进行纠错。函数插件贡献者: Binary-Husky"]
    )
    yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
