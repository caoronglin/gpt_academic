        ...
        results_final_sections.append(
            {
                "heading": k.lstrip("# "),
                "text": (
                    v if len(v) > 0 else f"The beginning of {k.lstrip('# ')} section."
                ),
            }
        )
...
def 批量翻译PDF文档(
    ...
    # 基本信息：功能、贡献者
    chatbot.append(["函数插件功能？", "批量翻译PDF文档。函数插件贡献者: Binary-Husky"])
    ...
    if len(file_manifest) == 0:
        report_exception(
            ...
            history,
            a=f"解析项目: {txt}",
            b=f"找不到任何.pdf拓展名的文件: {txt}",
        )
        yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
        return

    # 开始正式执行任务
    yield from 解析PDF_基于NOUGAT(
        file_manifest,
        project_folder,
        llm_kwargs,
        plugin_kwargs,
    ...
    for index, fp in enumerate(file_manifest):
        if fp.endswith("pdf"):
            chatbot.append(
                [
                    "当前进度：",
                    f"正在解析论文，请稍候。（第一次运行时，需要花费较长时间下载NOUGAT参数）",
                ]
            )
            yield from update_ui(chatbot=chatbot, history=history)  # 刷新界面
            fpp = yield from nougat_handle.NOUGAT_parse_pdf(fp, chatbot, history)
            promote_file_to_downloadzone(
                fpp, rename_file=os.path.basename(fpp) + ".nougat.mmd", chatbot=chatbot
            )
        else:
            chatbot.append(["当前论文无需解析：", fp])
            yield from update_ui(chatbot=chatbot, history=history)
            fpp = fp
