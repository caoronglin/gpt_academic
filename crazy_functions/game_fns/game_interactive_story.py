...
class MiniGame_ResumeStory(GptAcademicGameBaseState):
    ...
    def generate_story_image(self, story_paragraph):
        try:
            ...
            prompt_ = predict_no_ui_long_connection(
                inputs=story_paragraph,
                llm_kwargs=self.llm_kwargs,
                history=[],
                sys_prompt="你需要根据用户给出的小说段落，进行简短的环境描写。要求：80字以内。",
            )
    ...
    def step(self, prompt, chatbot, history):
        ...
        if self.step_cnt == 0:
            ...
            if prompt.strip() == "exit" or prompt.strip() == "结束剧情":
                ...
                yield from update_ui_latest_msg(
                    lastmsg=f"游戏结束。", chatbot=chatbot, history=history, delay=0.0
                )
                return
            if "剧情收尾" in prompt:
        ...
        if self.cur_task == "head_start":
            ...
            # # 配图
            yield from update_ui_latest_msg(
                lastmsg=story_paragraph + "<br/>正在生成插图中 ...",
                chatbot=chatbot,
                history=history,
                delay=0.0,
            )
            yield from update_ui_latest_msg(
                lastmsg=story_paragraph
                + "<br/>"
            ...
            self.next_choices = yield from request_gpt_model_in_new_thread_with_ui_alive(
                inputs_,
                "请在以下几种故事走向中，选择一种（当然，您也可以选择给出其他故事走向）：",
                self.llm_kwargs,
                chatbot,
                history_,
                self.sys_prompt_,
            ...
            根据用户的提示，确定故事的下一步
            """
            if "请在以下几种故事走向中，选择一种" in chatbot[-1][0]:
                chatbot.pop(-1)
            previously_on_story = ""
            for s in self.story:
                previously_on_story += s + "\n"
            inputs_ = prompts_resume.format(
                previously_on_story=previously_on_story,
                choice=self.next_choices,
                user_choice=prompt,
            )
            history_ = []
            story_paragraph = yield from request_gpt_model_in_new_thread_with_ui_alive(
                inputs_,
                f"下一段故事（您的选择是：{prompt}）。",
                self.llm_kwargs,
                chatbot,
                history_,
                self.sys_prompt_,
            ...
            self.next_choices = yield from request_gpt_model_in_new_thread_with_ui_alive(
                inputs_,
                "请在以下几种故事走向中，选择一种。当然，您也可以给出您心中的其他故事走向。另外，如果您希望剧情立即收尾，请输入剧情走向，并以“剧情收尾”四个字提示程序。",
                self.llm_kwargs,
                chatbot,
                history_,
                self.sys_prompt_,
            ...
            for s in self.story:
                previously_on_story += s + "\n"
            inputs_ = prompts_terminate.format(
                previously_on_story=previously_on_story, user_choice=prompt
            )
            history_ = []
            story_paragraph = yield from request_gpt_model_in_new_thread_with_ui_alive(
                inputs_,
