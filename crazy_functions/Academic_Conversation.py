...
def 学术对话(
    ...
    semantic_source = SemanticScholarSource(api_key=get_conf("SEMANTIC_SCHOLAR_KEY"))
    ...
    chatbot.append(
        [
            None,
            f"使用{handler.__class__.__name__}处理...，可能需要您耐心等待3～5分钟...",
        ]
    )
    yield from update_ui(chatbot=chatbot, history=history)
    ...
    if final_prompt:
        ...
        if papers_list:
            ...
            for idx, paper in enumerate(papers_list, 1):
                ...
                if paper.doi:
                    ...
                    doi_link = (
                        f" <a href='{doi_url}' target='_blank'>DOI: {paper.doi}</a>"
                    )
        ...
        try:
            ...
            result_file_md = write_history_to_file(
                history=[md_content], file_basename=f"markdown_{base_filename}.md"
            )
        ...
        try:
            ...
            result_file_html = write_history_to_file(
                history=[html_content], file_basename=f"html_{base_filename}.html"
            )
        ...
        if success_files:
            chatbot.append(
                [
                    "保存对话记录成功，bib和enw文件支持导入到EndNote、Zotero、JabRef、Mendeley等文献管理软件，HTML文件支持在浏览器中打开，里面包含详细论文源信息",
                    "对话已保存并添加到下载区，可以在下载区找到相关文件",
                ]
            )
        else:
