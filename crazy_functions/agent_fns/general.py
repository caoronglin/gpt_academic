...
class AutoGenGeneral(PluginMultiprocessManager):
    ...
    def exe_autogen(self, input):
        ...
        code_execution_config = {
            "work_dir": self.autogen_work_dir,
        ...
        for agent_kwargs in agents:
            ...
            for d in agent_handle._reply_func_list:
                if (
                    hasattr(d["reply_func"], "__name__")
                    and d["reply_func"].__name__ == "generate_oai_reply"
                ):
        ...
        with ProxyNetworkActivate("AutoGen"):
            ...
            try:
                groupchat = autogen.GroupChat(
                    agents=agents_instances, messages=[], max_round=50
                )
                ...
                self.child_conn.send(
                    PipeCom("done", "AutoGen exe failed: \n\n" + tb_str)
                )
